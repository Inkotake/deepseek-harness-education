...
describe('desktop Host plugin', () => {
  ...
  it('builds the loopback root with validated renderer mode and platform markers', () => {
    ...
    // The framed mode is the one that publishes the 36px content boundary on Windows. The removed
    // `extended` mode published it too, and the platform gate is covered by the Linux case below.
