...
| DSH host routing | [dsh-harness.md](references/dsh-harness.md) | The host is DeepSeek Harness and Node routing is recorded with `host-no-browser` evidence. |
...
2. Ask only for decisions that cannot be inferred. Combine the missing purpose/audience, theme, and scope/material decisions into one native form, grouped by section and with recommended defaults. Use the host's native question/form tool (in Codex, `request_user_input`; in DeepSeek Harness, `ask_user_question`) whenever available.
...
4. Build the normalized brief and slide-by-slide outline. Record each slide's role, composition intent, density, visual evidence, and source IDs. Persist `composition` and `density` only when they express a deliberate, recipe-compatible constraint; otherwise omit them and let the planner choose. They are runtime inputs, not commentary. Keep the detailed outline separate from the short decision summary.
...
6. After approval, choose the runtime:
   ...
   - In DeepSeek Harness, there is no browser tool; read [dsh-harness.md](references/dsh-harness.md), record `host-no-browser` routing evidence with `--browser-check not-required` and `--browser-step host-capability`, and route to the Node workflow. Do not fabricate `iab` or Chrome results.
...
- Use native PPTKit text, shapes, connectors, images, and tables. Bar, line, and pie charts are authored as native data-bound Chart IR v2 elements and remain editable in PowerPoint; keep their low-level styling theme-driven.
...
- Treat optional layout intents as constraints. Unless the outline deliberately requires a specific compatible treatment, omit `composition` and `density` and let the planner select from role, content, rhythm, theme, and seed.
