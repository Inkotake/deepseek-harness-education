...
test("renders Chart IR v2 bars and line markers in the SVG preview", async (t) => {
  const { server, url } = await serve();
  t.after(() => server.close());
  const browser = await chromium.launch({ headless: true });
  t.after(() => browser.close());
  ...
  session.deck.brief.slideCountRange = [4, 4];
  session.deck.slides.splice(1, 1,
    { id: "bar", role: "table", title: "Mix", chart: { type: "bar", categories: ["North", "South"], series: [{ name: "Direct", values: [35, 45] }, { name: "Partner", values: [65, 55] }], orientation: "horizontal", grouping: "percentStacked", axes: { value: { scale: { min: 0, max: 100, majorUnit: 20 } } } } },
    { id: "line", role: "table", title: "Trend", chart: { type: "line", categories: ["Jan", "Feb", "Mar"], series: [{ name: "Actual", values: [8, 6, 4], marker: "circle" }] } },
  );
  await sendSession(page, session);
  await page.waitForFunction(() => document.querySelectorAll("#thumbnails button").length === 4);
  await page.getByRole("button", { name: "Next" }).click();
  assert.equal(await page.locator('#stage [data-chart-role="bar"]').count(), 4);
  assert.ok(await page.locator('#stage [data-chart-role="gridline"]').count() > 0);
  await page.getByRole("button", { name: "Next" }).click();
  assert.equal(await page.locator('#stage [data-chart-role="line"]').count(), 1);
  assert.equal(await page.locator('#stage [data-chart-role="series-marker"]').count(), 3);
  assert.equal((await domBridge(page)).state.preview.status, "ready");
});
