# Third-Party Notices — Teacher DSH 0.1

Teacher DSH is a redistribution that bundles the components below. Each component remains under its own license; the Teacher DSH layer itself is MIT.

Verbatim license texts are collected in [`licenses/`](./licenses).

Generated: 2026-09-10T14:03:43.449Z

## Bundled components

| Component | Version | License | Role |
|---|---|---|---|
| DeepSeek Harness | 0.1.2-rc.1 | MIT | Agent runtime, plugin loader, client UI, and tool protocol |
| DSH Desktop (anywhere-labs) | v2.0.5 | MIT | Electron shell, profile manager, and packaging pipeline this distribution builds on |
| Node.js | 22.23.2 | MIT | Bundled JavaScript runtime used by every Teacher command |
| pnpm | 11.8.0 | MIT | Bundled package manager for artifact projects |
| Education Agent Skills (Gareth Manning) | pinned commit | CC BY-SA 4.0 | Seven vendored education Skills, redistributed unmodified |
| PPTKit Presentation | pinned commit | MIT | Vendored presentation workflow Skill and the PPTKit DSH plugin |
| Vercel Web Design Guidelines | pinned commit | MIT | Vendored design guidelines Skill snapshot |
| DSH Cowork | pinned commit | MIT | Office document read/write tools (xlsx, pdf, docx, pptx, ipynb) |
| DSH Better Sidebar | 0.18.1 | MIT | Explorer / editor / terminal / git / browser sidebar |

## Share-alike components

### Education Agent Skills (Gareth Manning) — CC BY-SA 4.0

Education Agent Skills by Gareth Manning, licensed CC BY-SA 4.0. The skills are redistributed verbatim; see the LICENSE and SOURCE.json inside each vendored skill directory. Any redistribution of this bundle must keep the same license for these files and credit the original author.

## Bundled npm dependencies

The following packages are installed into the shipped runtime trees. Each entry keeps the license recorded in its own `package.json`.

### artifact toolchain (141)

| Package | Version | License |
|---|---|---|
| @antfu/install-pkg | 2.0.1 | MIT |
| @braintree/sanitize-url | 7.1.2 | MIT |
| @chevrotain/cst-dts-gen | 11.1.2 | Apache-2.0 |
| @chevrotain/gast | 11.1.2 | Apache-2.0 |
| @chevrotain/regexp-to-ast | 11.1.2 | Apache-2.0 |
| @chevrotain/types | 11.1.2 | Apache-2.0 |
| @chevrotain/utils | 11.1.2 | Apache-2.0 |
| @iconify/types | 2.0.0 | MIT |
| @iconify/utils | 3.1.7 | MIT |
| @mermaid-js/parser | 2.0.0 | MIT |
| @oxc-project/types | 0.149.0 | MIT |
| @rolldown/binding-win32-x64-msvc | 1.2.8 | MIT |
| @rolldown/pluginutils | 1.0.1 | MIT |
| @teacher-dsh/artifact-sdk | 0.1.0 | MIT |
| @types/d3 | 7.4.3 | MIT |
| @types/d3-array | 3.2.2 | MIT |
| @types/d3-axis | 3.0.6 | MIT |
| @types/d3-brush | 3.0.6 | MIT |
| @types/d3-chord | 3.0.6 | MIT |
| @types/d3-color | 3.1.3 | MIT |
| @types/d3-contour | 3.0.6 | MIT |
| @types/d3-delaunay | 6.0.4 | MIT |
| @types/d3-dispatch | 3.0.7 | MIT |
| @types/d3-drag | 3.0.7 | MIT |
| @types/d3-dsv | 3.0.7 | MIT |
| @types/d3-ease | 3.0.2 | MIT |
| @types/d3-fetch | 3.0.7 | MIT |
| @types/d3-force | 3.0.10 | MIT |
| @types/d3-format | 3.0.4 | MIT |
| @types/d3-geo | 3.1.1 | MIT |
| @types/d3-hierarchy | 3.1.7 | MIT |
| @types/d3-interpolate | 3.0.4 | MIT |
| @types/d3-path | 3.1.1 | MIT |
| @types/d3-polygon | 3.0.2 | MIT |
| @types/d3-quadtree | 3.0.6 | MIT |
| @types/d3-random | 3.0.4 | MIT |
| @types/d3-scale | 4.0.9 | MIT |
| @types/d3-scale-chromatic | 3.1.0 | MIT |
| @types/d3-selection | 3.0.11 | MIT |
| @types/d3-shape | 3.2.0 | MIT |
| @types/d3-time | 3.0.4 | MIT |
| @types/d3-time-format | 4.0.3 | MIT |
| @types/d3-timer | 3.0.2 | MIT |
| @types/d3-transition | 3.0.9 | MIT |
| @types/d3-zoom | 3.0.8 | MIT |
| @types/geojson | 7946.0.16 | MIT |
| @types/trusted-types | 2.0.7 | MIT |
| @upsetjs/venn.js | 2.0.0 | MIT |
| chevrotain | 11.1.2 | Apache-2.0 |
| commander | 15.0.0 | MIT |
| commander | 7.2.0 | MIT |
| commander | 8.3.0 | MIT |
| cose-base | 1.0.3 | MIT |
| cose-base | 2.2.0 | MIT |
| cytoscape | 3.34.3 | MIT |
| cytoscape-cose-bilkent | 4.1.0 | MIT |
| cytoscape-fcose | 2.2.0 | MIT |
| d3 | 7.9.0 | ISC |
| d3-array | 2.12.1 | BSD-3-Clause |
| d3-array | 3.2.4 | ISC |
| d3-axis | 3.0.0 | ISC |
| d3-brush | 3.0.0 | ISC |
| d3-chord | 3.0.1 | ISC |
| d3-color | 3.1.0 | ISC |
| d3-contour | 4.0.2 | ISC |
| d3-delaunay | 6.0.4 | ISC |
| d3-dispatch | 3.0.1 | ISC |
| d3-drag | 3.0.0 | ISC |
| d3-dsv | 3.0.1 | ISC |
| d3-ease | 3.0.1 | BSD-3-Clause |
| d3-fetch | 3.0.1 | ISC |
| d3-force | 3.0.0 | ISC |
| d3-format | 3.1.2 | ISC |
| d3-geo | 3.1.1 | ISC |
| d3-hierarchy | 3.1.2 | ISC |
| d3-interpolate | 3.0.1 | ISC |
| d3-path | 1.0.9 | BSD-3-Clause |
| d3-path | 3.1.0 | ISC |
| d3-polygon | 3.0.1 | ISC |
| d3-quadtree | 3.0.1 | ISC |
| d3-random | 3.0.1 | ISC |
| d3-sankey | 0.12.3 | BSD-3-Clause |
| d3-scale | 4.0.2 | ISC |
| d3-scale-chromatic | 3.1.0 | ISC |
| d3-selection | 3.0.0 | ISC |
| d3-shape | 1.3.7 | BSD-3-Clause |
| d3-shape | 3.2.0 | ISC |
| d3-time | 3.1.0 | ISC |
| d3-time-format | 4.1.0 | ISC |
| d3-timer | 3.0.1 | ISC |
| d3-transition | 3.0.1 | ISC |
| d3-zoom | 3.0.0 | ISC |
| dagre-d3-es | 7.0.14 | MIT |
| dayjs | 1.11.23 | MIT |
| delaunator | 5.1.0 | ISC |
| detect-libc | 2.1.2 | Apache-2.0 |
| dompurify | 3.4.15 | (MPL-2.0 OR Apache-2.0) |
| echarts | 6.1.0 | Apache-2.0 |
| elkjs | 0.9.3 | EPL-2.0 |
| es-toolkit | 1.52.0 | MIT |
| fdir | 6.5.0 | MIT |
| hachure-fill | 0.5.2 | MIT |
| iconv-lite | 0.6.3 | MIT |
| import-meta-resolve | 4.2.0 | MIT |
| internmap | 1.0.1 | ISC |
| internmap | 2.0.3 | ISC |
| jsxgraph | 1.13.3 | (MIT OR LGPL-3.0-or-later) |
| katex | 0.16.47 | MIT |
| katex | 0.18.7 | MIT |
| khroma | 2.1.0 | UNKNOWN |
| layout-base | 1.0.2 | MIT |
| layout-base | 2.0.1 | MIT |
| lightningcss | 1.33.0 | MPL-2.0 |
| lightningcss-win32-x64-msvc | 1.33.0 | MPL-2.0 |
| lodash-es | 4.17.23 | MIT |
| marked | 16.4.2 | MIT |
| matter-js | 0.20.0 | MIT |
| mermaid | 12.0.0 | MIT |
| nanoid | 3.3.18 | MIT |
| package-manager-detector | 1.8.0 | MIT |
| path-data-parser | 0.1.0 | MIT |
| picocolors | 1.1.1 | ISC |
| picomatch | 4.0.7 | MIT |
| points-on-curve | 0.2.0 | MIT |
| points-on-path | 0.2.1 | MIT |
| postcss | 8.5.28 | MIT |
| robust-predicates | 3.0.3 | Unlicense |
| rolldown | 1.2.8 | MIT |
| roughjs | 4.6.6 | MIT |
| rw | 1.3.3 | BSD-3-Clause |
| safer-buffer | 2.1.2 | MIT |
| source-map-js | 1.2.1 | BSD-3-Clause |
| stylis | 4.4.0 | MIT |
| three | 0.186.0 | MIT |
| tinyexec | 1.3.1 | MIT |
| tinyglobby | 0.2.17 | MIT |
| ts-dedent | 2.3.0 | MIT |
| tslib | 2.3.0 | 0BSD |
| uuid | 14.0.2 | MIT |
| vite | 8.2.2 | MIT |
| zrender | 6.1.0 | BSD-3-Clause |

### deploy toolchain (1145)

| Package | Version | License |
|---|---|---|
| @babel/code-frame | 7.29.7 | MIT |
| @babel/helper-string-parser | 7.29.7 | MIT |
| @babel/helper-validator-identifier | 7.29.7 | MIT |
| @babel/parser | 7.29.8 | MIT |
| @babel/types | 7.29.8 | MIT |
| @bugsnag/browser | 8.10.0 | MIT |
| @bugsnag/core | 8.9.0 | MIT |
| @bugsnag/cuid | 3.2.2 | MIT |
| @bugsnag/js | 8.10.0 | MIT |
| @bugsnag/node | 8.10.0 | MIT |
| @bugsnag/safe-json-stringify | 6.1.0 | MIT |
| @bytecodealliance/preview2-shim | 0.17.6 | (Apache-2.0 WITH LLVM-exception) |
| @cloudflare/kv-asset-handler | 0.5.0 | MIT OR Apache-2.0 |
| @cloudflare/unenv-preset | 2.16.1 | MIT OR Apache-2.0 |
| @cloudflare/workerd-windows-64 | 1.20260908.1 | Apache-2.0 |
| @colors/colors | 1.6.0 | MIT |
| @colors/colors | 1.6.1 | MIT |
| @cspotcode/source-map-support | 0.8.1 | MIT |
| @dabh/diagnostics | 2.0.9 | MIT |
| @dependents/detective-less | 5.0.3 | MIT |
| @edge-runtime/format | 2.2.1 | MPL-2.0 |
| @edge-runtime/node-utils | 2.3.0 | MPL-2.0 |
| @edge-runtime/ponyfill | 2.4.2 | MPL-2.0 |
| @edge-runtime/primitives | 4.1.0 | MPL-2.0 |
| @edge-runtime/vm | 3.2.0 | MPL-2.0 |
| @electric-sql/pglite | 0.3.16 | Apache-2.0 |
| @emnapi/core | 1.11.3 | MIT |
| @emnapi/runtime | 1.11.3 | MIT |
| @emnapi/wasi-threads | 1.2.3 | MIT |
| @envelop/instrumentation | 1.0.0 | MIT |
| @esbuild/win32-x64 | 0.27.0 | MIT |
| @esbuild/win32-x64 | 0.27.7 | MIT |
| @esbuild/win32-x64 | 0.28.1 | MIT |
| @fastify/accept-negotiator | 2.1.0 | MIT |
| @fastify/ajv-compiler | 4.0.6 | MIT |
| @fastify/busboy | 2.1.1 | MIT |
| @fastify/busboy | 3.2.2 | MIT |
| @fastify/error | 4.2.0 | MIT |
| @fastify/fast-json-stringify-compiler | 5.1.0 | MIT |
| @fastify/forwarded | 3.0.2 | MIT |
| @fastify/merge-json-schemas | 0.2.1 | MIT |
| @fastify/proxy-addr | 5.1.0 | MIT |
| @fastify/send | 4.1.1 | MIT |
| @fastify/static | 10.1.3 | MIT |
| @humanwhocodes/momoa | 2.0.4 | Apache-2.0 |
| @img/colour | 1.1.0 | MIT |
| @img/sharp-win32-x64 | 0.34.5 | Apache-2.0 AND LGPL-3.0-or-later |
| @img/sharp-win32-x64 | 0.35.2 | Apache-2.0 AND LGPL-3.0-or-later |
| @import-maps/resolve | 2.0.0 | MIT |
| @inquirer/external-editor | 1.0.3 | MIT |
| @isaacs/balanced-match | 4.0.1 | MIT |
| @isaacs/brace-expansion | 5.0.1 | MIT |
| @isaacs/fs-minipass | 4.0.1 | ISC |
| @jridgewell/resolve-uri | 3.1.2 | MIT |
| @jridgewell/sourcemap-codec | 1.6.0 | MIT |
| @jridgewell/trace-mapping | 0.3.9 | MIT |
| @lukeed/ms | 2.0.2 | MIT |
| @mapbox/node-pre-gyp | 2.0.3 | BSD-3-Clause |
| @napi-rs/keyring | 1.2.0 | MIT |
| @napi-rs/keyring-win32-x64-msvc | 1.2.0 | MIT |
| @napi-rs/wasm-runtime | 1.2.4 | MIT |
| @netlify/ai | 1.0.1 | MIT |
| @netlify/api | 15.1.2 | MIT |
| @netlify/binary-info | 1.0.0 | Apache 2 |
| @netlify/blobs | 10.7.13 | MIT |
| @netlify/blobs | 11.0.3 | MIT |
| @netlify/build | 36.4.8 | MIT |
| @netlify/build-info | 11.3.0 | MIT |
| @netlify/cache | 4.0.0 | MIT |
| @netlify/cache-utils | 7.1.2 | MIT |
| @netlify/config | 25.2.5 | MIT |
| @netlify/database-dev | 1.0.1 | MIT |
| @netlify/dev | 5.0.5 | MIT |
| @netlify/dev-utils | 5.0.0 | MIT |
| @netlify/dev-utils | 6.0.1 | MIT |
| @netlify/edge-bundler | 16.0.4 | MIT |
| @netlify/edge-functions | 4.0.0 | MIT |
| @netlify/edge-functions-bootstrap | 2.16.0 | MIT |
| @netlify/edge-functions-bootstrap | 3.2.0 | MIT |
| @netlify/edge-functions-dev | 2.0.1 | MIT |
| @netlify/functions | 6.0.0 | MIT |
| @netlify/functions-dev | 2.0.5 | MIT |
| @netlify/functions-utils | 7.1.9 | MIT |
| @netlify/git-utils | 7.1.1 | MIT |
| @netlify/headers | 3.0.1 | MIT |
| @netlify/headers-parser | 10.1.1 | MIT |
| @netlify/images | 2.0.1 | MIT |
| @netlify/local-functions-proxy | 2.0.3 | MIT |
| @netlify/local-functions-proxy-win32-x64 | 1.1.1 | MIT |
| @netlify/open-api | 2.57.1 | MIT |
| @netlify/opentelemetry-utils | 3.1.0 | MIT |
| @netlify/otel | 6.0.6 | MIT |
| @netlify/otel | 7.0.2 | MIT |
| @netlify/plugins-list | 6.81.9 | MIT |
| @netlify/redirect-parser | 16.1.1 | MIT |
| @netlify/redirects | 4.0.2 | MIT |
| @netlify/run-utils | 7.1.0 | MIT |
| @netlify/runtime | 5.0.3 | MIT |
| @netlify/runtime-utils | 2.3.0 | MIT |
| @netlify/runtime-utils | 3.0.0 | MIT |
| @netlify/serverless-functions-api | 2.18.0 | MIT |
| @netlify/static | 4.0.0 | MIT |
| @netlify/types | 2.8.0 | MIT |
| @netlify/types | 3.0.0 | MIT |
| @netlify/zip-it-and-ship-it | 15.5.1 | MIT |
| @nodelib/fs.scandir | 2.1.5 | MIT |
| @nodelib/fs.stat | 2.0.5 | MIT |
| @nodelib/fs.walk | 1.2.8 | MIT |
| @octokit/auth-token | 6.0.0 | MIT |
| @octokit/core | 7.0.8 | MIT |
| @octokit/endpoint | 11.0.5 | MIT |
| @octokit/graphql | 9.0.5 | MIT |
| @octokit/openapi-types | 27.0.0 | MIT |
| @octokit/openapi-types | 29.0.1 | MIT |
| @octokit/plugin-paginate-rest | 14.0.0 | MIT |
| @octokit/plugin-request-log | 6.0.0 | MIT |
| @octokit/plugin-rest-endpoint-methods | 17.0.0 | MIT |
| @octokit/request | 10.0.16 | MIT |
| @octokit/request-error | 7.1.2 | MIT |
| @octokit/rest | 22.0.1 | MIT |
| @octokit/types | 16.0.0 | MIT |
| @octokit/types | 18.0.0 | MIT |
| @opentelemetry/api | 1.8.0 | Apache-2.0 |
| @opentelemetry/api | 1.9.1 | Apache-2.0 |
| @opentelemetry/api-logs | 0.220.0 | Apache-2.0 |
| @opentelemetry/api-logs | 0.221.0 | Apache-2.0 |
| @opentelemetry/context-async-hooks | 2.10.0 | Apache-2.0 |
| @opentelemetry/context-async-hooks | 2.9.0 | Apache-2.0 |
| @opentelemetry/core | 2.10.0 | Apache-2.0 |
| @opentelemetry/core | 2.8.0 | Apache-2.0 |
| @opentelemetry/core | 2.9.0 | Apache-2.0 |
| @opentelemetry/instrumentation | 0.220.0 | Apache-2.0 |
| @opentelemetry/instrumentation | 0.221.0 | Apache-2.0 |
| @opentelemetry/resources | 2.10.0 | Apache-2.0 |
| @opentelemetry/resources | 2.9.0 | Apache-2.0 |
| @opentelemetry/sdk-trace | 2.10.0 | Apache-2.0 |
| @opentelemetry/sdk-trace | 2.9.0 | Apache-2.0 |
| @opentelemetry/sdk-trace-base | 2.10.0 | Apache-2.0 |
| @opentelemetry/sdk-trace-base | 2.9.0 | Apache-2.0 |
| @opentelemetry/sdk-trace-node | 2.10.0 | Apache-2.0 |
| @opentelemetry/sdk-trace-node | 2.9.0 | Apache-2.0 |
| @opentelemetry/semantic-conventions | 1.43.0 | Apache-2.0 |
| @oxc-parser/binding-win32-x64-msvc | 0.121.0 | MIT |
| @oxc-project/types | 0.110.0 | MIT |
| @oxc-project/types | 0.121.0 | MIT |
| @oxc-transform/binding-win32-x64-msvc | 0.111.0 | MIT |
| @parcel/watcher-wasm | 2.6.0 | MIT |
| @pinojs/redact | 0.4.0 | MIT |
| @pnpm/config.env-replace | 1.1.0 | MIT |
| @pnpm/network.ca-file | 1.0.2 | MIT |
| @pnpm/npm-conf | 3.0.3 | MIT |
| @pnpm/tabtab | 0.5.4 | MIT |
| @poppinss/colors | 4.1.6 | MIT |
| @poppinss/dumper | 0.6.5 | MIT |
| @poppinss/exception | 1.2.3 | MIT |
| @renovatebot/pep440 | 4.2.1 | Apache-2.0 |
| @rolldown/binding-win32-x64-msvc | 1.0.0-rc.1 | MIT |
| @rolldown/pluginutils | 1.0.0-rc.1 | MIT |
| @rollup/pluginutils | 5.4.0 | MIT |
| @sec-ant/readable-stream | 0.4.1 | MIT |
| @sinclair/typebox | 0.25.24 | MIT |
| @sindresorhus/is | 7.2.0 | MIT |
| @sindresorhus/slugify | 2.2.1 | MIT |
| @sindresorhus/transliterate | 1.6.0 | MIT |
| @so-ric/colorspace | 1.1.6 | MIT |
| @speed-highlight/core | 1.2.24 | CC0-1.0 |
| @sveltejs/acorn-typescript | 1.0.13 | MIT |
| @tootallnate/once | 2.0.0 | MIT |
| @ts-morph/common | 0.11.1 | MIT |
| @tsconfig/node10 | 1.0.13 | MIT |
| @tsconfig/node12 | 1.0.11 | MIT |
| @tsconfig/node14 | 1.0.3 | MIT |
| @tsconfig/node16 | 1.0.4 | MIT |
| @tybys/wasm-util | 0.10.3 | MIT |
| @types/estree | 1.0.9 | MIT |
| @types/http-proxy | 1.17.17 | MIT |
| @types/json-schema | 7.0.15 | MIT |
| @types/node | 20.11.0 | MIT |
| @types/node | 22.20.2 | MIT |
| @types/normalize-package-data | 2.4.4 | MIT |
| @types/retry | 0.12.2 | MIT |
| @types/triple-beam | 1.3.5 | MIT |
| @typescript-eslint/project-service | 8.70.0 | MIT |
| @typescript-eslint/tsconfig-utils | 8.70.0 | MIT |
| @typescript-eslint/types | 8.70.0 | MIT |
| @typescript-eslint/typescript-estree | 8.70.0 | MIT |
| @typescript-eslint/visitor-keys | 8.70.0 | MIT |
| @vercel/backends | 7.0.4 | Apache-2.0 |
| @vercel/blob | 2.8.0 | Apache-2.0 |
| @vercel/build-utils | 14.9.3 | Apache-2.0 |
| @vercel/cervel | 0.1.63 | Apache-2.0 |
| @vercel/cli-auth | 0.3.7 | UNKNOWN |
| @vercel/cli-config | 0.2.6 | Apache-2.0 |
| @vercel/cli-exec | 1.0.1 | Apache-2.0 |
| @vercel/container | 7.0.3 | Apache-2.0 |
| @vercel/detect-agent | 1.2.5 | Apache-2.0 |
| @vercel/elysia | 7.0.3 | Apache-2.0 |
| @vercel/error-utils | 2.2.1 | Apache-2.0 |
| @vercel/express | 7.0.4 | Apache-2.0 |
| @vercel/fastify | 7.0.3 | Apache-2.0 |
| @vercel/fun | 1.3.0 | Apache-2.0 |
| @vercel/gatsby-plugin-vercel-analytics | 1.0.12 | Apache-2.0 |
| @vercel/gatsby-plugin-vercel-builder | 2.2.55 | Apache-2.0 |
| @vercel/go | 10.1.1 | Apache-2.0 |
| @vercel/h3 | 7.0.3 | Apache-2.0 |
| @vercel/hono | 7.0.3 | Apache-2.0 |
| @vercel/hydrogen | 8.0.3 | Apache-2.0 |
| @vercel/koa | 7.0.3 | Apache-2.0 |
| @vercel/nestjs | 7.0.3 | Apache-2.0 |
| @vercel/next | 11.0.4 | Apache-2.0 |
| @vercel/nft | 1.10.0 | MIT |
| @vercel/nft | 1.3.1 | MIT |
| @vercel/node | 12.0.3 | Apache-2.0 |
| @vercel/oidc | 3.2.0 | Apache-2.0 |
| @vercel/oidc | 3.8.7 | Apache-2.0 |
| @vercel/prepare-flags-definitions | 0.3.0 | MIT |
| @vercel/python | 13.0.4 | Apache-2.0 |
| @vercel/python-analysis | 0.14.0 | Apache-2.0 |
| @vercel/redwood | 9.0.3 | Apache-2.0 |
| @vercel/remix-builder | 12.0.3 | Apache-2.0 |
| @vercel/ruby | 9.0.3 | Apache-2.0 |
| @vercel/rust | 8.0.3 | Apache-2.0 |
| @vercel/sandbox | 3.1.0 | Apache-2.0 |
| @vercel/static-build | 9.0.3 | Apache-2.0 |
| @vercel/static-config | 3.4.3 | Apache-2.0 |
| @vue/compiler-core | 3.5.42 | MIT |
| @vue/compiler-dom | 3.5.42 | MIT |
| @vue/compiler-sfc | 3.5.42 | MIT |
| @vue/compiler-ssr | 3.5.42 | MIT |
| @vue/shared | 3.5.42 | MIT |
| @whatwg-node/disposablestack | 0.0.6 | MIT |
| @whatwg-node/fetch | 0.10.13 | MIT |
| @whatwg-node/node-fetch | 0.8.6 | MIT |
| @whatwg-node/promise-helpers | 1.3.2 | MIT |
| @whatwg-node/server | 0.11.0 | MIT |
| @workflow/serde | 4.1.0-beta.2 | Apache-2.0 |
| abbrev | 3.0.1 | ISC |
| abort-controller | 3.0.0 | MIT |
| abstract-logging | 2.0.1 | MIT |
| accepts | 2.0.0 | MIT |
| acorn | 8.18.0 | MIT |
| acorn-import-attributes | 1.9.5 | MIT |
| acorn-walk | 8.3.5 | MIT |
| agent-base | 7.1.4 | MIT |
| agent-base | 8.0.0 | MIT |
| ajv | 8.20.0 | MIT |
| ajv | 8.6.3 | MIT |
| ajv-errors | 3.0.0 | MIT |
| ajv-formats | 3.0.1 | MIT |
| ansi-align | 3.0.1 | ISC |
| ansi-colors | 4.1.3 | MIT |
| ansi-escapes | 4.3.2 | MIT |
| ansi-escapes | 7.3.0 | MIT |
| ansi-regex | 5.0.1 | MIT |
| ansi-regex | 6.3.0 | MIT |
| ansi-styles | 4.3.0 | MIT |
| ansi-styles | 6.2.3 | MIT |
| ansi-to-html | 0.7.2 | MIT |
| ansis | 4.3.1 | ISC |
| any-promise | 1.3.0 | MIT |
| anymatch | 3.1.3 | ISC |
| archiver | 8.0.0 | MIT |
| arg | 4.1.0 | MIT |
| arg | 4.1.3 | MIT |
| argparse | 2.0.1 | Python-2.0 |
| array-buffer-byte-length | 1.0.2 | MIT |
| array-timsort | 1.0.3 | MIT |
| array.prototype.flatmap | 1.3.3 | MIT |
| arraybuffer.prototype.slice | 1.0.4 | MIT |
| ascii-table | 0.0.9 | MIT |
| ast-module-types | 6.0.2 | MIT |
| async | 3.2.6 | MIT |
| async-function | 1.0.0 | MIT |
| async-listen | 1.2.0 | MIT |
| async-listen | 3.0.0 | MIT |
| async-listen | 3.0.1 | MIT |
| async-retry | 1.3.3 | MIT |
| async-sema | 3.1.1 | MIT |
| atomic-sleep | 1.0.0 | MIT |
| atomically | 2.1.1 | MIT |
| available-typed-arrays | 1.0.7 | MIT |
| avvio | 9.3.0 | MIT |
| b4a | 1.8.1 | Apache-2.0 |
| backoff | 2.5.0 | MIT |
| balanced-match | 1.0.2 | MIT |
| balanced-match | 4.0.4 | MIT |
| bare-events | 2.9.2 | Apache-2.0 |
| bare-fs | 4.8.1 | Apache-2.0 |
| bare-path | 3.1.2 | Apache-2.0 |
| bare-stream | 2.13.4 | Apache-2.0 |
| bare-url | 2.5.4 | Apache-2.0 |
| base64-js | 1.5.1 | MIT |
| before-after-hook | 4.0.0 | Apache-2.0 |
| better-ajv-errors | 1.2.0 | Apache-2.0 |
| bindings | 1.5.0 | MIT |
| bl | 4.1.0 | MIT |
| blake3-wasm | 2.1.5 | MIT |
| body-parser | 2.3.0 | MIT |
| boolbase | 1.0.0 | ISC |
| boxen | 8.0.1 | MIT |
| brace-expansion | 1.1.18 | MIT |
| brace-expansion | 5.0.9 | MIT |
| braces | 3.0.3 | MIT |
| buffer | 5.7.1 | MIT |
| buffer | 6.0.3 | MIT |
| buffer-crc32 | 0.2.13 | MIT |
| buffer-crc32 | 1.0.0 | MIT |
| buffer-equal-constant-time | 1.0.1 | BSD-3-Clause |
| buffer-from | 1.1.2 | MIT |
| bundle-name | 4.1.0 | MIT |
| byline | 5.0.0 | MIT |
| bytes | 3.1.0 | MIT |
| bytes | 3.1.2 | MIT |
| call-bind | 1.0.9 | MIT |
| call-bind-apply-helpers | 1.0.2 | MIT |
| call-bound | 1.0.4 | MIT |
| callsite | 1.0.0 | UNKNOWN |
| camelcase | 8.0.0 | MIT |
| chalk | 4.1.2 | MIT |
| chalk | 5.6.2 | MIT |
| chalk | 6.0.0 | MIT |
| chardet | 2.2.0 | MIT |
| chokidar | 4.0.0 | MIT |
| chokidar | 4.0.3 | MIT |
| chokidar | 5.0.0 | MIT |
| chownr | 3.0.0 | BlueOak-1.0.0 |
| ci-info | 4.4.0 | MIT |
| citty | 0.1.6 | MIT |
| citty | 0.2.2 | MIT |
| cjs-module-lexer | 1.2.3 | MIT |
| cjs-module-lexer | 2.2.1 | MIT |
| clean-deep | 3.4.0 | MIT |
| cli-boxes | 3.0.0 | MIT |
| cli-cursor | 3.1.0 | MIT |
| cli-cursor | 5.0.0 | MIT |
| cli-spinners | 2.9.2 | MIT |
| cli-width | 3.0.0 | ISC |
| cliui | 8.0.1 | ISC |
| clone | 1.0.4 | MIT |
| code-block-writer | 10.1.1 | MIT |
| color | 5.0.3 | MIT |
| color-convert | 2.0.1 | MIT |
| color-convert | 3.1.3 | MIT |
| color-name | 1.1.4 | MIT |
| color-name | 2.1.1 | MIT |
| color-string | 2.1.4 | MIT |
| colors | 1.4.0 | MIT |
| commander | 10.0.1 | MIT |
| commander | 11.1.0 | MIT |
| commander | 12.1.0 | MIT |
| commander | 2.20.3 | MIT |
| comment-json | 5.0.0 | MIT |
| common-path-prefix | 3.0.0 | ISC |
| compress-commons | 7.0.1 | MIT |
| concat-map | 0.0.1 | MIT |
| config-chain | 1.1.13 | MIT |
| configstore | 7.1.0 | BSD-2-Clause |
| consola | 3.4.2 | MIT |
| content-disposition | 1.1.0 | MIT |
| content-disposition | 2.0.1 | MIT |
| content-type | 1.0.4 | MIT |
| content-type | 1.0.5 | MIT |
| content-type | 2.1.0 | MIT |
| content-type | 3.0.0 | MIT |
| convert-hrtime | 3.0.0 | MIT |
| cookie | 0.7.2 | MIT |
| cookie | 1.1.1 | MIT |
| cookie | 2.0.1 | MIT |
| cookie-es | 1.2.3 | MIT |
| cookie-signature | 1.2.2 | MIT |
| copy-file | 11.1.0 | MIT |
| core-util-is | 1.0.3 | MIT |
| crc-32 | 1.2.2 | Apache-2.0 |
| crc32-stream | 7.0.1 | MIT |
| create-require | 1.1.1 | MIT |
| cron-parser | 5.10.0 | MIT |
| cross-spawn | 7.0.6 | MIT |
| crossws | 0.3.5 | MIT |
| crossws | 0.4.12 | MIT |
| css-select | 6.0.0 | BSD-2-Clause |
| css-tree | 2.2.1 | MIT |
| css-tree | 3.2.1 | MIT |
| css-what | 7.0.0 | BSD-2-Clause |
| cssfilter | 0.0.10 | MIT |
| csso | 5.0.5 | MIT |
| cyclist | 1.0.2 | MIT |
| data-uri-to-buffer | 4.0.1 | MIT |
| data-view-buffer | 1.0.2 | MIT |
| data-view-byte-length | 1.0.2 | MIT |
| data-view-byte-offset | 1.0.1 | MIT |
| debug | 4.3.4 | MIT |
| debug | 4.4.3 | MIT |
| decache | 4.6.2 | MIT |
| deep-extend | 0.6.0 | MIT |
| deepmerge | 4.3.1 | MIT |
| default-browser | 5.5.1 | MIT |
| default-browser-id | 5.0.1 | MIT |
| defaults | 1.0.4 | MIT |
| define-data-property | 1.1.4 | MIT |
| define-lazy-prop | 2.0.0 | MIT |
| define-lazy-prop | 3.0.0 | MIT |
| define-properties | 1.2.1 | MIT |
| defu | 6.1.7 | MIT |
| depd | 1.1.2 | MIT |
| depd | 2.0.0 | MIT |
| dequal | 2.0.3 | MIT |
| destr | 2.0.5 | MIT |
| detect-libc | 2.1.2 | Apache-2.0 |
| detective-amd | 6.1.0 | MIT |
| detective-cjs | 6.1.1 | MIT |
| detective-es6 | 5.0.2 | MIT |
| detective-postcss | 8.0.4 | MIT |
| detective-sass | 6.0.2 | MIT |
| detective-scss | 5.0.2 | MIT |
| detective-stylus | 5.0.1 | MIT |
| detective-typescript | 14.1.2 | MIT |
| detective-vue2 | 2.3.0 | MIT |
| dettle | 1.0.5 | MIT |
| diff | 4.0.4 | BSD-3-Clause |
| dom-serializer | 2.0.0 | MIT |
| domelementtype | 2.3.0 | BSD-2-Clause |
| domhandler | 5.0.3 | BSD-2-Clause |
| domutils | 3.2.2 | BSD-2-Clause |
| dot-prop | 10.2.0 | MIT |
| dot-prop | 9.0.0 | MIT |
| dotenv | 16.6.1 | BSD-2-Clause |
| dotenv | 17.4.2 | BSD-2-Clause |
| dunder-proto | 1.0.1 | MIT |
| ecdsa-sig-formatter | 1.0.11 | Apache-2.0 |
| edge-runtime | 2.5.9 | MPL-2.0 |
| ee-first | 1.1.1 | MIT |
| emoji-regex | 10.6.0 | MIT |
| emoji-regex | 8.0.0 | MIT |
| empathic | 2.0.1 | MIT |
| enabled | 2.0.0 | MIT |
| encodeurl | 2.0.0 | MIT |
| end-of-stream | 1.1.0 | MIT |
| end-of-stream | 1.4.5 | MIT |
| enquirer | 2.4.1 | MIT |
| entities | 2.2.0 | BSD-2-Clause |
| entities | 4.5.0 | BSD-2-Clause |
| entities | 7.0.1 | BSD-2-Clause |
| env-paths | 3.0.0 | MIT |
| env-paths | 4.0.0 | MIT |
| envinfo | 7.21.0 | MIT |
| environment | 1.1.0 | MIT |
| error-stack-parser | 2.1.4 | MIT |
| error-stack-parser-es | 1.0.5 | MIT |
| es-abstract | 1.24.2 | MIT |
| es-abstract-get | 1.0.0 | MIT |
| es-define-property | 1.0.1 | MIT |
| es-errors | 1.3.0 | MIT |
| es-module-lexer | 1.4.1 | MIT |
| es-module-lexer | 1.5.0 | MIT |
| es-module-lexer | 1.7.0 | MIT |
| es-module-lexer | 3.0.2 | MIT |
| es-object-atoms | 1.1.2 | MIT |
| es-set-tostringtag | 2.1.0 | MIT |
| es-shim-unscopables | 1.1.0 | MIT |
| es-to-primitive | 1.3.4 | MIT |
| esbuild | 0.27.0 | MIT |
| esbuild | 0.27.7 | MIT |
| esbuild | 0.28.1 | MIT |
| escalade | 3.2.0 | MIT |
| escape-goat | 4.0.0 | MIT |
| escape-html | 1.0.3 | MIT |
| escape-string-regexp | 1.0.5 | MIT |
| escape-string-regexp | 5.0.0 | MIT |
| escodegen | 2.1.0 | BSD-2-Clause |
| eslint-visitor-keys | 5.0.1 | Apache-2.0 |
| esprima | 4.0.1 | BSD-2-Clause |
| estraverse | 5.3.0 | BSD-2-Clause |
| estree-walker | 2.0.2 | MIT |
| esutils | 2.0.3 | BSD-2-Clause |
| etag | 1.8.1 | MIT |
| event-target-shim | 5.0.1 | MIT |
| eventemitter3 | 4.0.7 | MIT |
| events | 3.3.0 | MIT |
| events-intercept | 2.0.0 | MIT |
| events-universal | 1.0.1 | Apache-2.0 |
| execa | 3.2.0 | MIT |
| execa | 5.1.1 | MIT |
| execa | 8.0.1 | MIT |
| express | 5.2.1 | MIT |
| express-logging | 1.1.1 | Apache-2.0 |
| fast-decode-uri-component | 1.0.1 | MIT |
| fast-deep-equal | 3.1.3 | MIT |
| fast-equals | 5.4.2 | MIT |
| fast-fifo | 1.3.2 | MIT |
| fast-glob | 3.3.3 | MIT |
| fast-json-stringify | 7.0.1 | MIT |
| fast-querystring | 1.1.2 | MIT |
| fast-safe-stringify | 2.1.1 | MIT |
| fast-string-truncated-width | 3.0.3 | MIT |
| fast-string-width | 3.0.2 | MIT |
| fast-stringify | 4.0.2 | MIT |
| fast-uri | 3.1.7 | BSD-3-Clause |
| fast-uri | 4.1.4 | BSD-3-Clause |
| fastest-levenshtein | 1.0.16 | MIT |
| fastify | 5.12.3 | MIT |
| fastify-plugin | 6.0.0 | MIT |
| fastq | 1.20.3 | ISC |
| fd-slicer | 1.1.0 | MIT |
| fdir | 6.5.0 | MIT |
| fecha | 4.2.3 | MIT |
| fetch-blob | 3.2.0 | MIT |
| figures | 3.2.0 | MIT |
| figures | 6.1.0 | MIT |
| file-uri-to-path | 1.0.0 | MIT |
| fill-range | 7.1.1 | MIT |
| filter-obj | 6.1.0 | MIT |
| finalhandler | 2.1.1 | MIT |
| find-my-way | 9.9.0 | MIT |
| find-up | 7.0.0 | MIT |
| find-up | 8.0.0 | MIT |
| find-up-simple | 1.0.1 | MIT |
| fn.name | 1.1.0 | MIT |
| folder-walker | 3.2.0 | BSD-2-Clause |
| follow-redirects | 1.16.0 | MIT |
| for-each | 0.3.5 | MIT |
| formdata-polyfill | 4.0.10 | MIT |
| forwarded | 0.2.0 | MIT |
| fresh | 2.0.0 | MIT |
| from2 | 2.3.0 | MIT |
| fs-extra | 11.1.0 | MIT |
| fs-extra | 11.1.1 | MIT |
| function-bind | 1.1.2 | MIT |
| function.prototype.name | 1.2.0 | MIT |
| functions-have-names | 1.2.3 | MIT |
| fuzzy | 0.1.3 | MIT |
| generator-function | 2.0.1 | MIT |
| generic-pool | 3.4.2 | MIT |
| get-amd-module-type | 6.0.2 | MIT |
| get-caller-file | 2.0.5 | ISC |
| get-east-asian-width | 1.6.0 | MIT |
| get-intrinsic | 1.3.0 | MIT |
| get-port | 5.1.1 | MIT |
| get-port | 7.2.0 | MIT |
| get-port-please | 3.2.0 | MIT |
| get-proto | 1.0.1 | MIT |
| get-stream | 5.2.0 | MIT |
| get-stream | 6.0.1 | MIT |
| get-stream | 8.0.1 | MIT |
| get-stream | 9.0.1 | MIT |
| get-symbol-description | 1.1.0 | MIT |
| get-tsconfig | 4.14.3 | MIT |
| git-repo-info | 2.1.1 | MIT |
| gitconfiglocal | 2.1.0 | BSD |
| glob | 13.0.6 | BlueOak-1.0.0 |
| glob-parent | 5.1.2 | ISC |
| global-directory | 4.0.1 | MIT |
| globalthis | 1.0.4 | MIT |
| gonzales-pe | 4.3.0 | MIT |
| gopd | 1.2.0 | MIT |
| graceful-fs | 4.2.10 | ISC |
| graceful-fs | 4.2.11 | ISC |
| h3 | 1.15.11 | MIT |
| has-bigints | 1.1.0 | MIT |
| has-flag | 4.0.0 | MIT |
| has-flag | 5.0.1 | MIT |
| has-property-descriptors | 1.0.2 | MIT |
| has-proto | 1.2.0 | MIT |
| has-symbols | 1.1.0 | MIT |
| has-tostringtag | 1.0.2 | MIT |
| hasown | 2.0.4 | MIT |
| hosted-git-info | 7.0.2 | ISC |
| hosted-git-info | 8.1.0 | ISC |
| hosted-git-info | 9.0.3 | ISC |
| hot-shots | 11.4.0 | MIT |
| http-errors | 1.7.3 | MIT |
| http-errors | 2.0.0 | MIT |
| http-errors | 2.0.1 | MIT |
| http-proxy | 1.18.1 | MIT |
| http-proxy-middleware | 3.0.7 | MIT |
| http-shutdown | 1.2.2 | MIT |
| https-proxy-agent | 7.0.6 | MIT |
| https-proxy-agent | 8.0.0 | MIT |
| human-signals | 1.1.1 | Apache-2.0 |
| human-signals | 2.1.0 | Apache-2.0 |
| human-signals | 5.0.0 | Apache-2.0 |
| iconv-lite | 0.4.24 | MIT |
| iconv-lite | 0.7.3 | MIT |
| ieee754 | 1.2.1 | BSD-3-Clause |
| ignore | 7.0.9 | MIT |
| image-meta | 0.2.2 | MIT |
| import-in-the-middle | 3.5.0 | Apache-2.0 |
| imurmurhash | 0.1.4 | MIT |
| indent-string | 5.0.0 | MIT |
| index-to-position | 1.2.0 | MIT |
| inherits | 2.0.4 | ISC |
| ini | 1.3.8 | ISC |
| ini | 4.1.1 | ISC |
| inquirer | 8.2.7 | MIT |
| inquirer-autocomplete-prompt | 1.4.0 | ISC |
| internal-slot | 1.1.0 | MIT |
| ipaddr.js | 1.9.1 | MIT |
| ipaddr.js | 2.5.0 | MIT |
| ipx | 3.1.1 | MIT |
| iron-webcrypto | 1.2.1 | MIT |
| is-array-buffer | 3.0.5 | MIT |
| is-async-function | 2.1.1 | MIT |
| is-bigint | 1.1.0 | MIT |
| is-boolean-object | 1.2.2 | MIT |
| is-buffer | 2.0.5 | MIT |
| is-callable | 1.2.7 | MIT |
| is-core-module | 2.16.2 | MIT |
| is-data-view | 1.0.2 | MIT |
| is-date-object | 1.1.0 | MIT |
| is-docker | 2.2.1 | MIT |
| is-docker | 3.0.0 | MIT |
| is-docker | 4.0.0 | MIT |
| is-document.all | 1.0.0 | MIT |
| is-error-instance | 2.0.0 | MIT |
| is-extglob | 2.1.1 | MIT |
| is-finalizationregistry | 1.1.1 | MIT |
| is-fullwidth-code-point | 3.0.0 | MIT |
| is-fullwidth-code-point | 5.1.0 | MIT |
| is-generator-function | 1.1.2 | MIT |
| is-glob | 4.0.3 | MIT |
| is-in-ci | 1.0.0 | MIT |
| is-in-ssh | 1.0.0 | MIT |
| is-inside-container | 1.0.0 | MIT |
| is-installed-globally | 1.0.0 | MIT |
| is-interactive | 1.0.0 | MIT |
| is-map | 2.0.3 | MIT |
| is-negative-zero | 2.0.3 | MIT |
| is-network-error | 1.3.2 | MIT |
| is-node-process | 1.2.0 | MIT |
| is-npm | 6.1.0 | MIT |
| is-number | 7.0.0 | MIT |
| is-number-object | 1.1.1 | MIT |
| is-path-inside | 4.0.0 | MIT |
| is-plain-obj | 2.1.0 | MIT |
| is-plain-obj | 4.1.0 | MIT |
| is-plain-object | 5.1.0 | MIT |
| is-promise | 4.0.0 | MIT |
| is-regex | 1.2.1 | MIT |
| is-safe-filename | 0.1.1 | MIT |
| is-set | 2.0.3 | MIT |
| is-shared-array-buffer | 1.0.4 | MIT |
| is-stream | 2.0.1 | MIT |
| is-stream | 3.0.0 | MIT |
| is-stream | 4.0.1 | MIT |
| is-string | 1.1.1 | MIT |
| is-symbol | 1.1.1 | MIT |
| is-typed-array | 1.1.15 | MIT |
| is-unicode-supported | 0.1.0 | MIT |
| is-unicode-supported | 2.1.0 | MIT |
| is-url-superb | 4.0.0 | MIT |
| is-weakmap | 2.0.2 | MIT |
| is-weakref | 1.1.1 | MIT |
| is-weakset | 2.0.4 | MIT |
| is-wsl | 2.2.0 | MIT |
| is-wsl | 3.1.1 | MIT |
| isarray | 1.0.0 | MIT |
| isarray | 2.0.5 | MIT |
| iserror | 0.0.2 | MIT |
| isexe | 2.0.0 | ISC |
| isexe | 4.0.0 | BlueOak-1.0.0 |
| jiti | 2.7.0 | MIT |
| jose | 5.9.6 | MIT |
| jose | 6.2.3 | MIT |
| js-tokens | 4.0.0 | MIT |
| js-yaml | 4.1.1 | MIT |
| json-schema-ref-resolver | 3.0.0 | MIT |
| json-schema-to-ts | 1.6.4 | MIT |
| json-schema-traverse | 1.0.0 | MIT |
| json-with-bigint | 3.5.12 | MIT |
| jsonc-parser | 3.3.1 | MIT |
| jsonfile | 6.2.1 | MIT |
| jsonlines | 0.1.1 | MIT |
| jsonpointer | 5.0.1 | MIT |
| jsonwebtoken | 9.0.3 | MIT |
| junk | 4.0.1 | MIT |
| jwa | 2.0.1 | MIT |
| jws | 4.0.1 | MIT |
| jwt-decode | 4.0.0 | MIT |
| keep-func-props | 6.0.0 | MIT |
| kleur | 4.1.5 | MIT |
| kuler | 2.0.0 | MIT |
| ky | 1.14.3 | MIT |
| lambda-local | 2.2.0 | MIT |
| latest-version | 9.0.0 | MIT |
| lazystream | 1.0.1 | MIT |
| leven | 3.1.0 | MIT |
| light-my-request | 6.6.0 | BSD-3-Clause |
| listhen | 1.10.1 | MIT |
| locate-path | 7.2.0 | MIT |
| locate-path | 8.0.0 | MIT |
| lockparse | 0.5.2 | MIT |
| lodash | 4.18.1 | MIT |
| lodash.includes | 4.3.0 | MIT |
| lodash.isboolean | 3.0.3 | MIT |
| lodash.isempty | 4.4.0 | MIT |
| lodash.isinteger | 4.0.4 | MIT |
| lodash.isnumber | 3.0.3 | MIT |
| lodash.isplainobject | 4.0.6 | MIT |
| lodash.isstring | 4.0.1 | MIT |
| lodash.once | 4.1.1 | MIT |
| lodash.transform | 4.6.0 | MIT |
| log-process-errors | 11.0.1 | Apache-2.0 |
| log-symbols | 4.1.0 | MIT |
| log-update | 8.0.0 | MIT |
| logform | 2.7.0 | MIT |
| lru-cache | 10.4.3 | ISC |
| lru-cache | 11.5.2 | BlueOak-1.0.0 |
| lru-cache | 6.0.0 | ISC |
| luxon | 3.7.2 | MIT |
| macos-release | 3.5.1 | MIT |
| magic-string | 0.30.21 | MIT |
| make-error | 1.3.6 | ISC |
| map-obj | 5.0.2 | MIT |
| math-intrinsics | 1.1.0 | MIT |
| maxstache | 1.0.7 | MIT |
| maxstache-stream | 1.0.4 | MIT |
| mdn-data | 2.0.28 | CC0-1.0 |
| mdn-data | 2.27.1 | CC0-1.0 |
| media-typer | 1.1.1 | MIT |
| memoize-one | 6.0.0 | MIT |
| merge-descriptors | 2.0.0 | MIT |
| merge-options | 3.0.4 | MIT |
| merge-stream | 2.0.0 | MIT |
| merge2 | 1.4.1 | MIT |
| micro | 9.3.5-canary.3 | MIT |
| micro-memoize | 5.2.0 | MIT |
| micromatch | 4.0.8 | MIT |
| mime | 3.0.0 | MIT |
| mime-db | 1.52.0 | MIT |
| mime-db | 1.54.0 | MIT |
| mime-types | 2.1.35 | MIT |
| mime-types | 3.0.2 | MIT |
| mimic-fn | 2.1.0 | MIT |
| mimic-fn | 4.0.0 | MIT |
| mimic-function | 5.0.1 | MIT |
| miniflare | 5.20260908.0-alpha | MIT |
| minimatch | 10.1.1 | BlueOak-1.0.0 |
| minimatch | 10.2.6 | BlueOak-1.0.0 |
| minimatch | 3.1.5 | ISC |
| minimist | 1.2.8 | MIT |
| minipass | 7.1.3 | BlueOak-1.0.0 |
| minizlib | 3.1.0 | MIT |
| mkdirp | 1.0.4 | MIT |
| modern-tar | 0.8.5 | MIT |
| module-definition | 6.0.2 | MIT |
| module-details-from-path | 1.0.4 | MIT |
| mri | 1.2.0 | MIT |
| ms | 2.1.1 | MIT |
| ms | 2.1.2 | MIT |
| ms | 2.1.3 | MIT |
| multiparty | 4.3.0 | MIT |
| mute-stream | 0.0.8 | ISC |
| nanoid | 3.3.18 | MIT |
| nanospinner | 1.2.2 | MIT |
| napi-wasm | 1.1.0 | MIT |
| negotiator | 1.1.0 | MIT |
| netlify-cli | 27.5.2 | MIT |
| netlify-redirector | 0.5.0 | MIT |
| node-domexception | 1.0.0 | MIT |
| node-exports-info | 1.6.2 | MIT |
| node-fetch | 2.6.7 | MIT |
| node-fetch | 2.6.9 | MIT |
| node-fetch | 2.7.0 | MIT |
| node-fetch | 3.3.2 | MIT |
| node-fetch-native | 1.6.7 | MIT |
| node-forge | 1.4.0 | (BSD-3-Clause OR GPL-2.0) |
| node-gyp-build | 4.8.4 | MIT |
| node-mock-http | 1.0.5 | MIT |
| node-source-walk | 7.0.2 | MIT |
| node-stream-zip | 1.16.0 | MIT |
| nopt | 8.1.0 | ISC |
| normalize-exception | 3.0.0 | MIT |
| normalize-package-data | 6.0.2 | BSD-2-Clause |
| normalize-package-data | 7.0.1 | BSD-2-Clause |
| normalize-package-data | 8.0.0 | BSD-2-Clause |
| normalize-path | 2.1.1 | MIT |
| normalize-path | 3.0.0 | MIT |
| npm-run-path | 4.0.1 | MIT |
| npm-run-path | 5.3.0 | MIT |
| nth-check | 2.1.1 | BSD-2-Clause |
| object-inspect | 1.13.4 | MIT |
| object-keys | 1.1.1 | MIT |
| object.assign | 4.1.7 | MIT |
| object.entries | 1.1.9 | MIT |
| ofetch | 1.5.1 | MIT |
| on-exit-leak-free | 2.1.2 | MIT |
| on-finished | 2.4.1 | MIT |
| on-headers | 1.1.0 | MIT |
| once | 1.3.3 | ISC |
| once | 1.4.0 | ISC |
| one-time | 1.0.0 | MIT |
| onetime | 5.1.2 | MIT |
| onetime | 6.0.0 | MIT |
| onetime | 7.0.0 | MIT |
| open | 11.0.2 | MIT |
| open | 8.4.0 | MIT |
| ora | 5.4.1 | MIT |
| os-name | 6.1.0 | MIT |
| os-paths | 4.4.0 | MIT |
| own-keys | 1.0.2 | MIT |
| oxc-parser | 0.121.0 | MIT |
| oxc-transform | 0.111.0 | MIT |
| p-event | 6.0.1 | MIT |
| p-filter | 4.1.0 | MIT |
| p-finally | 2.0.1 | MIT |
| p-limit | 4.0.0 | MIT |
| p-locate | 6.0.0 | MIT |
| p-map | 7.0.7 | MIT |
| p-reduce | 3.0.0 | MIT |
| p-retry | 6.2.1 | MIT |
| p-timeout | 6.1.4 | MIT |
| p-wait-for | 6.0.0 | MIT |
| package-directory | 8.2.0 | MIT |
| package-json | 10.0.1 | MIT |
| parallel-transform | 1.2.0 | MIT |
| parse-duration | 2.1.8 | MIT |
| parse-github-url | 1.0.4 | MIT |
| parse-gitignore | 2.0.0 | MIT |
| parse-imports | 2.2.1 | Apache-2.0 AND MIT |
| parse-json | 8.3.0 | MIT |
| parse-ms | 2.1.0 | MIT |
| parse-ms | 4.0.0 | MIT |
| parseurl | 1.3.3 | MIT |
| path-browserify | 1.0.1 | MIT |
| path-exists | 5.0.0 | MIT |
| path-key | 3.1.1 | MIT |
| path-key | 4.0.0 | MIT |
| path-parse | 1.0.7 | MIT |
| path-scurry | 2.0.2 | BlueOak-1.0.0 |
| path-to-regexp | 6.1.0 | MIT |
| path-to-regexp | 6.3.0 | MIT |
| path-to-regexp | 8.2.0 | MIT |
| path-to-regexp | 8.3.0 | MIT |
| path-to-regexp | 8.4.2 | MIT |
| path-type | 6.0.0 | MIT |
| pathe | 2.0.3 | MIT |
| pend | 1.2.0 | MIT |
| pg | 8.23.0 | MIT |
| pg-cloudflare | 1.4.0 | MIT |
| pg-connection-string | 2.14.0 | MIT |
| pg-gateway | 0.3.0-beta.4 | MIT |
| pg-int8 | 1.0.1 | ISC |
| pg-pool | 3.14.0 | MIT |
| pg-protocol | 1.16.0 | MIT |
| pg-types | 2.2.0 | MIT |
| pgpass | 1.0.5 | MIT |
| picocolors | 1.0.0 | ISC |
| picocolors | 1.1.1 | ISC |
| picomatch | 2.3.2 | MIT |
| picomatch | 4.0.7 | MIT |
| picoquery | 2.5.0 | MIT |
| pino | 10.3.1 | MIT |
| pino-abstract-transport | 3.0.0 | MIT |
| pino-std-serializers | 7.1.0 | MIT |
| possible-typed-array-names | 1.1.0 | MIT |
| postcss | 8.5.28 | MIT |
| postcss-values-parser | 6.0.2 | MPL-2.0 |
| postgres-array | 2.0.0 | MIT |
| postgres-bytea | 1.0.1 | MIT |
| postgres-date | 1.0.7 | MIT |
| postgres-interval | 1.2.0 | MIT |
| powershell-utils | 0.1.0 | MIT |
| powershell-utils | 0.2.1 | MIT |
| precinct | 12.3.2 | MIT |
| precond | 0.2.3 | UNKNOWN |
| pretty-ms | 7.0.1 | MIT |
| pretty-ms | 9.3.1 | MIT |
| prettyjson | 1.2.5 | MIT |
| process | 0.11.10 | MIT |
| process-nextick-args | 2.0.1 | MIT |
| process-warning | 4.0.1 | MIT |
| process-warning | 5.1.0 | MIT |
| promisepipe | 3.0.0 | MIT |
| proto-list | 1.2.4 | ISC |
| proxy-addr | 2.0.7 | MIT |
| ps-list | 8.1.1 | MIT |
| pump | 1.0.3 | MIT |
| pump | 3.0.4 | MIT |
| punycode | 2.3.1 | MIT |
| pupa | 3.3.0 | MIT |
| qs | 6.16.0 | BSD-3-Clause |
| queue-microtask | 1.2.3 | MIT |
| quick-format-unescaped | 4.0.4 | MIT |
| quote-unquote | 1.0.0 | MIT |
| radix3 | 1.1.2 | MIT |
| random-bytes | 1.0.0 | MIT |
| range-parser | 1.3.0 | MIT |
| raw-body | 2.4.1 | MIT |
| raw-body | 3.0.2 | MIT |
| raw-body | 4.0.0 | MIT |
| rc | 1.2.8 | (BSD-2-Clause OR MIT OR Apache-2.0) |
| read-package-up | 11.0.0 | MIT |
| read-package-up | 12.0.0 | MIT |
| read-pkg | 10.1.0 | MIT |
| read-pkg | 9.0.1 | MIT |
| readable-stream | 2.3.8 | MIT |
| readable-stream | 3.6.2 | MIT |
| readable-stream | 4.7.0 | MIT |
| readdir-glob | 3.0.0 | Apache-2.0 |
| readdirp | 4.1.2 | MIT |
| readdirp | 5.1.1 | MIT |
| real-require | 0.2.0 | MIT |
| real-require | 1.0.0 | MIT |
| reflect.getprototypeof | 1.0.10 | MIT |
| regexp.prototype.flags | 1.5.4 | MIT |
| registry-auth-token | 5.1.1 | MIT |
| registry-url | 6.0.1 | MIT |
| remove-trailing-separator | 1.1.0 | ISC |
| require-directory | 2.1.1 | MIT |
| require-from-string | 2.0.2 | MIT |
| require-in-the-middle | 8.0.1 | MIT |
| require-package-name | 2.0.1 | MIT |
| requires-port | 1.0.0 | MIT |
| resolve | 2.0.0-next.7 | MIT |
| resolve-from | 5.0.0 | MIT |
| resolve-pkg-maps | 1.0.0 | MIT |
| resolve.exports | 2.0.3 | MIT |
| restore-cursor | 3.1.0 | MIT |
| restore-cursor | 5.1.0 | MIT |
| ret | 0.5.0 | MIT |
| retry | 0.13.1 | MIT |
| reusify | 1.1.0 | MIT |
| rfdc | 1.4.1 | MIT |
| rolldown | 1.0.0-rc.1 | MIT |
| router | 2.2.0 | MIT |
| run-applescript | 7.1.0 | MIT |
| run-async | 2.4.1 | MIT |
| run-parallel | 1.2.0 | MIT |
| rxjs | 6.6.7 | Apache-2.0 |
| rxjs | 7.8.2 | Apache-2.0 |
| safe-array-concat | 1.1.4 | MIT |
| safe-buffer | 5.1.2 | MIT |
| safe-buffer | 5.2.1 | MIT |
| safe-json-stringify | 1.2.0 | MIT |
| safe-push-apply | 1.0.0 | MIT |
| safe-regex-test | 1.1.0 | MIT |
| safe-regex2 | 5.1.1 | MIT |
| safe-stable-stringify | 2.5.0 | MIT |
| safer-buffer | 2.1.2 | MIT |
| sandbox | 4.1.0 | Apache-2.0 |
| sax | 1.6.1 | BlueOak-1.0.0 |
| secure-json-parse | 4.1.0 | BSD-3-Clause |
| semver | 6.3.1 | ISC |
| semver | 7.5.4 | ISC |
| semver | 7.8.5 | ISC |
| send | 1.2.1 | MIT |
| serve-static | 2.2.1 | MIT |
| set-cookie-parser | 2.7.2 | MIT |
| set-error-message | 2.0.1 | MIT |
| set-function-length | 1.2.2 | MIT |
| set-function-name | 2.0.2 | MIT |
| set-proto | 1.0.0 | MIT |
| setprototypeof | 1.1.1 | ISC |
| setprototypeof | 1.2.0 | ISC |
| sharp | 0.34.5 | Apache-2.0 |
| sharp | 0.35.2 | Apache-2.0 |
| shebang-command | 2.0.0 | MIT |
| shebang-regex | 3.0.0 | MIT |
| side-channel | 1.1.1 | MIT |
| side-channel-list | 1.0.1 | MIT |
| side-channel-map | 1.0.1 | MIT |
| side-channel-weakmap | 1.0.2 | MIT |
| signal-exit | 3.0.7 | ISC |
| signal-exit | 4.0.2 | ISC |
| signal-exit | 4.1.0 | ISC |
| slashes | 3.0.12 | ISC |
| slice-ansi | 9.0.0 | MIT |
| smol-toml | 1.5.2 | BSD-3-Clause |
| smol-toml | 1.8.0 | BSD-3-Clause |
| sonic-boom | 4.2.1 | MIT |
| source-map | 0.6.1 | BSD-3-Clause |
| source-map-js | 1.2.1 | BSD-3-Clause |
| source-map-support | 0.5.21 | MIT |
| spdx-correct | 3.2.0 | Apache-2.0 |
| spdx-exceptions | 2.5.0 | CC-BY-3.0 |
| spdx-expression-parse | 3.0.1 | MIT |
| spdx-license-ids | 3.0.23 | CC0-1.0 |
| split2 | 1.1.1 | ISC |
| split2 | 4.2.0 | ISC |
| srvx | 0.11.16 | MIT |
| stack-generator | 2.0.10 | MIT |
| stack-trace | 0.0.10 | MIT |
| stackframe | 1.3.4 | MIT |
| stat-mode | 0.3.0 | MIT |
| statuses | 1.5.0 | MIT |
| statuses | 2.0.1 | MIT |
| statuses | 2.0.2 | MIT |
| std-env | 4.2.0 | MIT |
| stop-iteration-iterator | 1.1.0 | MIT |
| stream-to-array | 2.3.0 | MIT |
| stream-to-promise | 2.2.0 | MIT |
| streamx | 2.28.1 | MIT |
| string_decoder | 1.1.1 | MIT |
| string_decoder | 1.3.0 | MIT |
| string-width | 4.2.3 | MIT |
| string-width | 7.2.0 | MIT |
| string-width | 8.2.2 | MIT |
| string.prototype.trim | 1.2.11 | MIT |
| string.prototype.trimend | 1.0.10 | MIT |
| string.prototype.trimstart | 1.0.8 | MIT |
| strip-ansi | 6.0.1 | MIT |
| strip-ansi | 7.2.0 | MIT |
| strip-final-newline | 2.0.0 | MIT |
| strip-final-newline | 3.0.0 | MIT |
| strip-json-comments | 2.0.1 | MIT |
| stubborn-fs | 2.0.0 | MIT |
| stubborn-utils | 1.0.2 | MIT |
| supports-color | 10.2.2 | MIT |
| supports-color | 11.0.0 | MIT |
| supports-color | 7.2.0 | MIT |
| supports-hyperlinks | 3.2.0 | MIT |
| supports-hyperlinks | 4.5.0 | MIT |
| supports-preserve-symlinks-flag | 1.0.0 | MIT |
| svgo | 4.1.0 | MIT |
| tagged-tag | 1.0.0 | MIT |
| tar | 7.5.22 | BlueOak-1.0.0 |
| tar | 7.5.7 | BlueOak-1.0.0 |
| tar-stream | 3.1.7 | MIT |
| tar-stream | 3.2.1 | MIT |
| teex | 1.0.1 | MIT |
| terminal-link | 4.0.0 | MIT |
| terminal-link | 5.0.0 | MIT |
| text-decoder | 1.2.7 | Apache-2.0 |
| text-hex | 1.0.0 | MIT |
| thread-stream | 4.2.0 | MIT |
| throttleit | 2.1.0 | MIT |
| through | 2.3.8 | MIT |
| through2 | 2.0.5 | MIT |
| time-span | 4.0.0 | MIT |
| tinyclip | 0.1.15 | MIT |
| tinyexec | 0.3.2 | MIT |
| tinyexec | 1.3.1 | MIT |
| tinyglobby | 0.2.17 | MIT |
| tmp | 0.2.7 | MIT |
| tmp-promise | 3.0.3 | MIT |
| to-regex-range | 5.0.1 | MIT |
| toad-cache | 3.7.4 | MIT |
| toidentifier | 1.0.0 | MIT |
| toidentifier | 1.0.1 | MIT |
| toml | 4.3.0 | MIT |
| tomlify-j0.4 | 3.0.0 | MIT |
| tr46 | 0.0.3 | MIT |
| tree-kill | 1.2.2 | MIT |
| triple-beam | 1.4.1 | MIT |
| ts-api-utils | 2.5.0 | MIT |
| ts-morph | 12.0.0 | MIT |
| ts-node | 10.9.2 | MIT |
| ts-toolbelt | 6.15.5 | Apache-2.0 |
| tslib | 1.14.1 | 0BSD |
| tslib | 2.8.1 | 0BSD |
| tsx | 4.21.0 | MIT |
| type-fest | 0.21.3 | (MIT OR CC0-1.0) |
| type-fest | 4.41.0 | (MIT OR CC0-1.0) |
| type-fest | 5.9.0 | (MIT OR CC0-1.0) |
| type-is | 2.1.0 | MIT |
| typed-array-buffer | 1.0.3 | MIT |
| typed-array-byte-length | 1.0.3 | MIT |
| typed-array-byte-offset | 1.0.4 | MIT |
| typed-array-length | 1.0.8 | MIT |
| typescript | 5.9.3 | Apache-2.0 |
| ufo | 1.6.4 | MIT |
| uid-promise | 1.0.0 | MIT |
| uid-safe | 2.1.5 | MIT |
| ulid | 3.0.2 | MIT |
| unbox-primitive | 1.1.0 | MIT |
| uncrypto | 0.1.3 | MIT |
| undici | 5.28.4 | MIT |
| undici | 5.29.0 | MIT |
| undici | 6.28.1 | MIT |
| undici | 7.29.0 | MIT |
| undici | 7.29.1 | MIT |
| undici-types | 5.26.5 | MIT |
| undici-types | 6.21.0 | MIT |
| unenv | 2.0.0-rc.24 | MIT |
| unicorn-magic | 0.1.0 | MIT |
| unicorn-magic | 0.3.0 | MIT |
| unicorn-magic | 0.4.0 | MIT |
| universal-user-agent | 7.0.3 | ISC |
| universalify | 2.0.1 | MIT |
| unixify | 1.0.0 | MIT |
| unpipe | 1.0.0 | MIT |
| unstorage | 1.17.5 | MIT |
| untildify | 4.0.0 | MIT |
| untun | 0.2.2 | MIT |
| update-notifier | 7.3.1 | BSD-2-Clause |
| uqr | 0.1.3 | MIT |
| uri-js | 4.4.1 | BSD-2-Clause |
| urlpattern-polyfill | 10.1.0 | MIT |
| urlpattern-polyfill | 8.0.2 | MIT |
| util-deprecate | 1.0.2 | MIT |
| uuid | 14.0.1 | MIT |
| v8-compile-cache-lib | 3.0.1 | MIT |
| validate-npm-package-license | 3.0.4 | Apache-2.0 |
| validate-npm-package-name | 5.0.1 | ISC |
| vary | 1.1.2 | MIT |
| vercel | 59.15.1 | Apache-2.0 |
| wcwidth | 1.0.1 | MIT |
| web-streams-polyfill | 3.3.3 | MIT |
| web-vitals | 0.2.4 | Apache-2.0 |
| webidl-conversions | 3.0.1 | BSD-2-Clause |
| whatwg-url | 5.0.0 | MIT |
| when-exit | 2.1.5 | MIT |
| which | 2.0.2 | ISC |
| which-boxed-primitive | 1.1.1 | MIT |
| which-builtin-type | 1.2.1 | MIT |
| which-collection | 1.0.2 | MIT |
| which-typed-array | 1.1.22 | MIT |
| widest-line | 5.0.0 | MIT |
| windows-release | 6.1.0 | MIT |
| winston | 3.19.0 | MIT |
| winston-transport | 4.9.0 | MIT |
| workerd | 1.20260908.1 | Apache-2.0 |
| wrangler | 4.130.0 | MIT OR Apache-2.0 |
| wrap-ansi | 10.0.1 | MIT |
| wrap-ansi | 6.2.0 | MIT |
| wrap-ansi | 7.0.0 | MIT |
| wrap-ansi | 9.0.2 | MIT |
| wrappy | 1.0.2 | ISC |
| write-file-atomic | 5.0.1 | ISC |
| ws | 8.21.0 | MIT |
| ws | 8.21.3 | MIT |
| wsl-utils | 1.0.0 | MIT |
| xdg-app-paths | 5.1.0 | MIT |
| xdg-app-paths | 5.5.1 | MIT |
| xdg-basedir | 5.1.0 | MIT |
| xdg-portable | 7.3.0 | MIT |
| xss | 1.0.15 | MIT |
| xtend | 4.0.2 | MIT |
| y18n | 5.0.8 | ISC |
| yallist | 4.0.0 | ISC |
| yallist | 5.0.0 | BlueOak-1.0.0 |
| yaml | 2.9.0 | ISC |
| yargs | 17.7.3 | MIT |
| yargs-parser | 21.1.1 | ISC |
| yauzl | 2.10.0 | MIT |
| yauzl | 3.4.0 | MIT |
| yauzl-clone | 1.0.4 | MIT |
| yauzl-promise | 2.1.3 | MIT |
| yn | 3.1.1 | MIT |
| yocto-queue | 1.2.2 | MIT |
| youch | 4.1.0-beta.10 | MIT |
| youch-core | 0.3.3 | MIT |
| zip-stream | 7.0.5 | MIT |
| zod | 3.22.4 | MIT |
| zod | 4.1.11 | MIT |
| zod | 4.6.1 | MIT |

### harness runtime (232)

| Package | Version | License |
|---|---|---|
| @deepseek-ai/cordis | 4.0.2 | MIT |
| @deepseek-ai/cordis-plugin-group | 1.0.2 | MIT |
| @deepseek-ai/cordis-plugin-hmr | 1.0.17 | MIT |
| @deepseek-ai/cordis-plugin-include | 1.0.7 | MIT |
| @deepseek-ai/cordis-plugin-loader | 1.0.3 | MIT |
| @deepseek-ai/cordis-plugin-timer | 1.1.4 | MIT |
| @deepseek-ai/cosmokit | 1.8.3 | MIT |
| @deepseek-ai/dsh | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-acp | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-acp-app | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-agent | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-agent-default-model | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-agent-instructions | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-agent-loop | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-agent-presets | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-agent-tool-presentation | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-anonymous-user-id | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-api-gateway | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-api-remotes | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-api-session-controller | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-api-settings-controller | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-api-workspace-controller | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-app-boot | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-atomic-write | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-attachment | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-attachment-local | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-authorization | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-base | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-bash-local | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-bash-sandbox | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-brand | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-connection | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-hmr | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-locale | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-modules | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-store | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-agent-preset | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-approval | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-attachment | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-brand-official | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-chat | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-commands | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-conversation | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-cordis | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-deliverables | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-directory-picker-browse | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-directory-picker-native | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-goal | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-input-trigger | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-jobs | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-layout | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-message-feedback | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-model-selection | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-permission-presets | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-plan | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-primitives | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-reference | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-renderer | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-schedule | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-session | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-settings | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-settings-general | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-settings-models | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-settings-plugin-inventory | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-settings-plugins | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-sidebar | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-skill | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-slots | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-subagent | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-theme | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-tool | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-trajectory | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-user-questions | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-workflow-run | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-client-ui-workspace | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-cmdline | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-code-runtime | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-code-runtime-worker-thread | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-command-compact | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-command-feedback | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-command-goal | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-commands | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-compaction | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-compaction-basic | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-compaction-tool-result-pruner | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-cordis-client-runner | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-cordis-host-runner | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-credentials | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-credentials-local | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-deepseek-llm-api-extensions | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-deque | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-file-reference | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-file-reference-local | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-fs | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-fs-local | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-fs-observation-policy | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-fs-sandbox | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-goal | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-goal-round-driver | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-headless | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-home-paths | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-hook-protocol | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-hooks-claude-code | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-hooks-codex | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-host-directory-picker | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-host-directory-picker-auto | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-host-directory-picker-browse | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-host-directory-picker-native | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-host-frontend-static | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-host-plugin-inventory | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-host-webserver | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-invariants | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-jobs | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-jobs-local | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-launch-environment | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-llm | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-llm-deepseek | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-llm-pi-ai | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-llm-retry | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-mcp-client | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-message-feedback | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-native-command | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-output-retention | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-permission-presets | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-persona | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-plan-mode | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-plugin-package-inventory-deepseek | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-pwsh-local | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-pwsh-sandbox | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-repeat-tool-reminder | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-sandbox | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-sandbox-local | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-sandbox-policy | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-sandbox-windows-acl | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-schedule | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-scope | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-sdk-app | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-sdk-jsonrpc-server | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-sdk-minimal | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-sdk-protocol | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-session | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-session-checkpoint-policy | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-session-log-deepseek | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-session-log-export | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-session-persistence | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-session-persistence-jsonl | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-session-projection | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-session-projection-cache | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-session-query | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-session-query-sqlite | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-session-reference | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-session-stats | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-session-telemetry | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-session-telemetry-otel | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-session-title | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-session-title-first-prompt-llm | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-session-title-llm | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-session-turn-outline | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-settings | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-settings-file | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-shell | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-shell-env | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-skill | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-skill-badge | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-skill-filesystem | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-spill | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-spill-local | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-spill-policy | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-storage | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-storage-domain | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-storage-json | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-subagent | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-subagent-fork-in-process | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-subagent-in-process-driver | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-subagent-spawn-in-process | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-subprocess | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-subprocess-local | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-system-prompt | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-terminal | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-terminal-bash | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-time-context | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-timeout | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-tmux-context | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-token-meter | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-tool-ask-user | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-tool-bash | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-tool-bash-persistent | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-tool-call-timeout-policy | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-tool-cordis | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-tool-fs | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-tool-fs-search | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-tool-goal | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-tool-jobs | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-tool-pwsh | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-tool-pwsh-persistent | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-tool-ralph | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-tool-skill | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-tool-str-replace-editor | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-tool-subagent | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-tool-subagent-control | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-tool-todo | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-tool-web | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-tool-workflow | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-tools | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-typert-loader | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-typert-protocol | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-typert-registry | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-user-approval | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-user-questions | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-util-crypto | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-util-time | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-util-values | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-util-workspace-path | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-web | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-web-app | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-web-fetch-http | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-web-frontend | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-web-search-deepseek | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-webhook | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-webhook-github | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-win32-process | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-workflow | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-workflow-worker-thread | 0.1.2-rc.1 | MIT |
| @deepseek-ai/dsh-workspace | 0.1.2-rc.1 | MIT |
| @deepseek-ai/node-addon-landlock-run | 0.1.1 | BSD-3-Clause |
| @deepseek-ai/schemastery | 3.18.2 | MIT |
| @opentelemetry/core | 2.10.0 | Apache-2.0 |
| @opentelemetry/resources | 2.10.0 | Apache-2.0 |
| chokidar | 5.0.0 | MIT |
| readdirp | 5.1.1 | MIT |
| undici | 8.10.0 | MIT |
| use-sync-external-store | 1.2.0 | MIT |

### presentation toolchain (19)

| Package | Version | License |
|---|---|---|
| @types/node | 22.20.2 | MIT |
| core-util-is | 1.0.3 | MIT |
| https | 1.0.0 | ISC |
| image-size | 1.2.1 | MIT |
| immediate | 3.0.6 | MIT |
| inherits | 2.0.4 | ISC |
| isarray | 1.0.0 | MIT |
| jszip | 3.10.2 | (MIT OR GPL-3.0-or-later) |
| lie | 3.3.0 | MIT |
| pako | 1.0.11 | (MIT AND Zlib) |
| pptxgenjs | 4.0.1 | MIT |
| process-nextick-args | 2.0.1 | MIT |
| queue | 6.0.2 | MIT |
| readable-stream | 2.3.8 | MIT |
| safe-buffer | 5.1.2 | MIT |
| setimmediate | 1.0.5 | MIT |
| string_decoder | 1.1.1 | MIT |
| undici-types | 6.21.0 | MIT |
| util-deprecate | 1.0.2 | MIT |

## Packages without a declared license

These packages do not declare a license in `package.json`. They are listed so the gap is visible; review them before publishing a release.

- @vercel/cli-auth@0.3.7 (deploy toolchain)
- callsite@1.0.0 (deploy toolchain)
- khroma@2.1.0 (artifact toolchain)
- precond@0.2.3 (deploy toolchain)

