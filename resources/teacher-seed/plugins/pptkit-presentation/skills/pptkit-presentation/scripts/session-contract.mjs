...
function validateChart(chart, path) {
  if (!record(chart) || !strings(chart.categories) || chart.categories.length === 0 || !Array.isArray(chart.series) || chart.series.length === 0) throw new Error(`${path} requires non-empty categories and series arrays.`);
  if (!CHART_TYPES.has(chart.type)) throw new Error(`${path}.type is unsupported: ${String(chart.type)}.`);
  ...
  if (chart.type === "bar" && chart.orientation !== undefined && !CHART_ORIENTATIONS.has(chart.orientation)) throw new Error(`${path}.orientation is unsupported: ${String(chart.orientation)}.`);
  if (chart.type === "bar" && chart.grouping !== undefined && !CHART_GROUPINGS.has(chart.grouping)) throw new Error(`${path}.grouping is unsupported: ${String(chart.grouping)}.`);
  ...
  if (chart.legend !== undefined) {
    if (!record(chart.legend)) throw new Error(`${path}.legend must be an object.`);
    ...
    if (chart.legend.position !== undefined && !CHART_LEGEND_POSITIONS.has(chart.legend.position)) throw new Error(`${path}.legend.position is unsupported: ${String(chart.legend.position)}.`);
  }
  if (chart.axes !== undefined) {
    if (!record(chart.axes)) throw new Error(`${path}.axes must be an object.`);
    for (const axisName of ["category", "value"]) {
      const axis = chart.axes[axisName];
      if (axis === undefined) continue;
      if (!record(axis)) throw new Error(`${path}.axes.${axisName} must be an object.`);
    ...
    if (record(valueAxis) && valueAxis.scale !== undefined && valueAxis.scale !== "auto") {
      const scale = valueAxis.scale;
      if (!record(scale) || !Number.isFinite(scale.min) || !Number.isFinite(scale.max) || scale.min >= scale.max || !Number.isFinite(scale.majorUnit) || scale.majorUnit <= 0) throw new Error(`${path}.axes.value.scale requires finite min < max and majorUnit > 0.`);
    }
  }
  for (const [seriesIndex, series] of chart.series.entries()) {
    if (!record(series)) throw new Error(`${path}.series[${seriesIndex}] must be an object.`);
    requiredString(series.name, `${path}.series[${seriesIndex}].name`);
    ...
    if (chart.type === "line" && series.marker !== undefined && series.marker !== false && !CHART_MARKERS.has(series.marker)) throw new Error(`${path}.series[${seriesIndex}].marker is unsupported: ${String(series.marker)}.`);
...
function validateSlide(slide, index, sourceIds, assetIds) {
  ...
  if (slide.chart !== undefined) validateChart(slide.chart, `${path}.chart`);
