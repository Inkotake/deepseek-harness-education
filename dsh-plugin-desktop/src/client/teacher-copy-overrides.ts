...
/** The one vendored key this distribution replaces, in both namespaces. */
export const FULL_ACCESS_WARNING_KEY = 'confirm.description'
...
const FULL_ACCESS_RISK_ITEMS_EN = [
  ...
  'Access to sensitive data: any file that account can read can be read, including API keys, access tokens, SSH private keys, browser profiles, mail, and chat data.',
  ...
  'Irreversible outcomes: a mistake or a misunderstood instruction can have consequences that cannot be undone, and earlier work cannot always be recovered.',
] as const
...
export function installFullAccessWarningCopy(ctx: ClientContext): () => void {
  ...
  const replacement = function replacement(
    ...
    if (key === FULL_ACCESS_WARNING_KEY) {
