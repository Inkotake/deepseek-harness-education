...
if (stablePlugin.name !== 'dsh-plugin-desktop') fail('the Desktop workspace must retain dsh-plugin-desktop')
// There is one upstream channel because there is one product. A `beta` channel used to name a
// second product package that was never shipped; keeping the channel after removing the product
// would leave `activeChannel` pointing at metadata for a workspace that no longer exists.
if (upstream.activeChannel !== 'stable') fail('the pinned upstream checkout must follow the stable channel')
if (Object.keys(upstream.channels ?? {}).length !== 1) {
  fail('upstream.json must declare exactly one channel')
}
