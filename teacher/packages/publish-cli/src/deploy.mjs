...
import { log, redact, statePath, warn } from './common.mjs';
...
export function claimOutput(claim) {
  if (!claim) return null;
  return {
    kind: claim.kind,
    value: claim.value ?? null,
    claimUrl: claim.claimUrl ?? null,
    warning: claim.warning || 'Do not forward the claim value or claim link: holding it means owning the deployment.'
  };
}
...
export async function deployArtifact(options) {
  ...
  if (onlyProvider) {
    ...
    if (!candidates.length) {
      ...
      return {
        ...
        artifactOk: true,
        reason,
        nextAction: 'run teacher-publish plan --json to see the eligible providers, or drop --provider',
        attempts: []
      };
    }
  }
  ...
  if (!candidates.length) {
    return {
      ...
      nextAction: 'run teacher-publish plan --json to see why each provider was excluded',
      attempts: [],
  ...
  for (const candidate of candidates) {
    ...
    if (verifyEnabled) {
      const providerHtmlPolicy = (provider.verification && provider.verification.htmlPolicy) || 'strict';
      try {
        verification = await verifyDeployment({
          publicUrl: deployment.url,
          manifest,
          options: { ...(options.verifyOptions || {}), htmlPolicy: providerHtmlPolicy }
        });
      } catch (cause) {
    ...
    return {
      ...
      expiresAtSource: deployment.expiresAtSource || (provider.capabilities?.ttlSecondsSource ?? null),
      claim: claimOutput(deployment.claim),
...
function nextActionFor(lastFailure, mode) {
  if (!lastFailure) return 'run teacher-publish plan --json to check provider eligibility';
  ...
  switch (lastFailure.kind) {
    ...
    default:
      return 'run teacher-publish plan --json and teacher-publish providers --json to inspect provider state';
  }
}
...
const TUNNEL_TOOLS = [
  { id: 'cloudflared', binaries: ['cloudflared'], hint: 'cloudflared tunnel --url http://localhost:PORT' },
  { id: 'localtunnel', binaries: ['lt'], hint: 'lt --port PORT' },
  { id: 'ngrok', binaries: ['ngrok'], hint: 'ngrok http PORT' }
];
...
export function detectTunnelTools(options = {}) {
  ...
  for (const tool of TUNNEL_TOOLS) {
    const binary = tool.binaries.find((name) => (findBinary ? findBinary(name) : whichSync(name)));
    if (binary) found.push({ id: tool.id, binary, hint: tool.hint });
    else missing.push({ id: tool.id, hint: tool.hint });
  }
