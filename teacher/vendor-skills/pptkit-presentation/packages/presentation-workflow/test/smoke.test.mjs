...
test("renders a native chart element instead of shape-simulated bars", () => {
  ...
  assert.equal(charts[0].orientation, "vertical");
  assert.equal(charts[0].grouping, "clustered");
  assert.equal(charts[0].legend.visible, false);
  assert.equal(charts[0].style.chartArea.type, "none");
  assert.equal(charts[0].style.plotArea.type, "none");
...
test("maps semantic ChartPlan options to Chart IR v2", () => {
  ...
  spec.slides[1] = {
    ...
    chart: {
      ...
      series: [{ name: "Direct", values: [35, 45] }, { name: "Partner", values: [65, 55] }],
      orientation: "horizontal",
      grouping: "percentStacked",
      legend: { position: "bottom" },
      axes: { category: { labels: false }, value: { scale: { min: 0, max: 100, majorUnit: 20 } } },
    },
  };
  assert.deepEqual(validateDeckSpec(spec), []);
  const chart = normalizePresentation(presentationOf(spec)).slides[1].elements.find((element) => element.type === "chart");
  assert.equal(chart.orientation, "horizontal");
  assert.equal(chart.grouping, "percentStacked");
  assert.equal(chart.legend.visible, true);
  assert.equal(chart.legend.position, "bottom");
  assert.equal(chart.axes.category.labels, false);
  assert.deepEqual(chart.axes.value.scale, { min: 0, max: 100, majorUnit: 20 });
  assert.equal(chart.style.textStyle.fontFamily, "Aptos");
});

test("maps line marker intents without exposing low-level styles", () => {
  const spec = deck();
  spec.slides[1] = {
    id: "line",
    role: "table",
    ...
    chart: {
      ...
      series: [{ name: "Actual", values: [8, 6, 4], marker: "circle" }, { name: "Target", values: [7, 5, 3], marker: false }],
    },
  };
  const chart = normalizePresentation(presentationOf(spec)).slides[1].elements.find((element) => element.type === "chart");
  assert.equal(chart.legend.visible, true);
  assert.equal(chart.legend.position, "bottom");
  assert.equal(chart.series[0].marker.shape, "circle");
  assert.equal(chart.series[1].marker, false);
});
...
test("rejects invalid Chart IR v2 data before authoring", () => {
  const mismatched = deck();
  mismatched.slides[1] = { id: "bar", role: "table", title: "Mismatch", chart: { type: "bar", categories: ["A", "B"], series: [{ name: "Value", values: [1] }] } };
  assert.ok(validateDeckSpec(mismatched).some((issue) => issue.code === "chart-data-length"));
  const negativePie = deck();
  negativePie.slides[1] = { id: "pie", role: "table", title: "Negative", chart: { type: "pie", categories: ["A", "B"], series: [{ name: "Value", values: [1, -1] }] } };
  assert.ok(validateDeckSpec(negativePie).some((issue) => issue.code === "pie-negative-value"));
  const invalidScale = deck();
  invalidScale.slides[1] = { id: "line", role: "table", title: "Scale", chart: { type: "line", categories: ["A"], series: [{ name: "Value", values: [1] }], axes: { value: { scale: { min: 10, max: 0, majorUnit: 0 } } } } };
  assert.ok(validateDeckSpec(invalidScale).some((issue) => issue.code === "invalid-chart-scale"));
  const crossedFields = deck();
  crossedFields.slides[1] = { id: "pie", role: "table", title: "Crossed", chart: { type: "pie", categories: ["A"], series: [{ name: "Value", values: [1], marker: "circle" }], axes: {} } };
  assert.equal(validateDeckSpec(crossedFields).filter((issue) => issue.code === "invalid-chart-field").length, 2);
});
