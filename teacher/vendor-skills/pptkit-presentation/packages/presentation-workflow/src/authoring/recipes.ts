...
function addChart(slide: PresentationSlide, tokens: ThemeTokens, chart: ChartPlan, box: Box) {
  ...
  const colors = [tokens.accent, tokens.accent2];
  const legend = {
    visible: chart.legend?.visible ?? (chart.type === "pie" || chart.series.length > 1),
    position: chart.legend?.position ?? (chart.type === "pie" ? "right" as const : "bottom" as const),
    textStyle: { fontFamily: tokens.bodyFont, fontSize: tokens.captionSize, color: tokens.muted },
  };
  const style = {
    chartArea: { type: "none" as const },
    plotArea: { type: "none" as const },
    textStyle: { fontFamily: tokens.bodyFont, fontSize: tokens.captionSize, color: tokens.muted },
  };
  const axisLine = { paint: { type: "solid" as const, color: tokens.muted, opacity: 0.5 }, width: 0.75 };
  const gridLine = { paint: { type: "solid" as const, color: tokens.muted, opacity: 0.18 }, width: 0.75 };
  const axes = {
    category: {
      visible: chart.axes?.category?.visible ?? true,
      labels: chart.axes?.category?.labels ?? true,
      line: axisLine,
      majorGridlines: false as const,
      labelStyle: { fontFamily: tokens.bodyFont, fontSize: tokens.captionSize, color: tokens.muted },
    },
    value: {
      visible: chart.axes?.value?.visible ?? true,
      labels: chart.axes?.value?.labels ?? true,
      line: axisLine,
      majorGridlines: gridLine,
      labelStyle: { fontFamily: tokens.bodyFont, fontSize: tokens.captionSize, color: tokens.muted },
      scale: chart.axes?.value?.scale ?? "auto" as const,
    },
  };
  const base = { type: "chart" as const, name: "Chart", categories, legend, style, box };
  ...
  if (chart.type === "bar") {
    slide.addElement({
      ...
      axes,
      series: chart.series.slice(0, 2).map((series, index) => ({ name: series.name, values: series.values.slice(0, categories.length), color: colors[index] ?? tokens.accent })),
    });
    return;
  }
  if (chart.type === "line") {
    slide.addElement({
      ...
      series: chart.series.slice(0, 2).map((series, index) => {
        const color = colors[index] ?? tokens.accent;
        const marker = typeof series.marker === "string" && series.marker !== "auto" ? { shape: series.marker, size: 6 } : series.marker;
        return { name: series.name, values: series.values.slice(0, categories.length), color, line: { paint: { type: "solid", color, opacity: 1 }, width: 1.75 }, ...(marker === undefined ? {} : { marker }) };
      }),
    });
    return;
  }
  slide.addElement({
    ...base,
    chartType: "pie",
    series: chart.series.slice(0, 1).map((series) => ({ name: series.name, values: series.values.slice(0, categories.length) })),
  });
}
