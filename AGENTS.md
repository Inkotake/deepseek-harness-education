...
- Initialize the pinned upstream checkout with `yarn fetch:upstream` (it is fetched into an
  ignored directory, deliberately not a Git submodule).
...
- `deepseek-harness/` is a fetched, ignored upstream checkout, NOT a Git submodule: this repository
  carries no upstream gitlink and no `.gitmodules`, so upstream's history and remotes stay out
  of it. Never edit files inside it from a desktop feature branch.
...
- The upstream checkout keeps its own pnpm workspace. Run upstream commands through the root `upstream:*` scripts, whose Yarn portable-shell commands enter the directory before invoking Corepack.
...
- Commit before major changes of direction and keep the upstream pin update separate from desktop behavior changes.
