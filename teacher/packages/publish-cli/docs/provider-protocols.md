# Teacher DSH static provider protocols

Verified reference for the anonymous / persistent static-hosting providers that
`teacher-publish` implements. This file is the contract the adapters in
`teacher/packages/publish-cli/src/providers/` must satisfy.

Everything below was verified against live documentation and, where noted, against published
source code. Anything not verified is marked **UNVERIFIED** and must not be relied on.
...
CLIs in `resources/teacher-runtime/deploy/node_modules/.bin`. They are only selected when the
...
`teacher/packages/publish-cli/tools/probe-contracts.mjs`. They refine, and in three cases
