...
- `chart`: optional on `table` roles; `chart.type` supports `"bar"`, `"line"`, or `"pie"`. Existing `{ type, categories, series }` plans remain valid. Bar charts may set `orientation` (`vertical` or `horizontal`) and `grouping` (`clustered`, `stacked`, or `percentStacked`). Bar and line charts may set `axes.category.visible`, `axes.category.labels`, `axes.value.visible`, `axes.value.labels`, and `axes.value.scale` (`auto` or `{ min, max, majorUnit }`). Line series may set `marker` to `auto`, `false`, or a supported marker shape. All chart types may set `legend.visible` and `legend.position` (`right` or `bottom`). Pie charts require exactly one non-negative series and never accept axes.
