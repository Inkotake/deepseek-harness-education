...
function addChartIssues(slide: SlidePlan, issues: StructuralIssue[]) {
  const chart = slide.chart;
  if (!chart) return;
  const rawChart = chart as unknown as Record<string, unknown>;
  if (chart.type !== "bar" && rawChart.orientation !== undefined) issues.push({ severity: "error", code: "invalid-chart-field", message: "Chart orientation is only supported for bar charts.", slideId: slide.id });
  if (chart.type !== "bar" && rawChart.grouping !== undefined) issues.push({ severity: "error", code: "invalid-chart-field", message: "Chart grouping is only supported for bar charts.", slideId: slide.id });
  if (chart.type === "pie" && rawChart.axes !== undefined) issues.push({ severity: "error", code: "invalid-chart-field", message: "Pie charts do not support axes.", slideId: slide.id });
  if (chart.categories.length === 0 || chart.series.length === 0) issues.push({ severity: "error", code: "chart-empty-data", message: "Charts require at least one category and one series.", slideId: slide.id });
  chart.series.forEach((series, seriesIndex) => {
    const rawSeries = series as unknown as Record<string, unknown>;
    if (series.values.length !== chart.categories.length) issues.push({ severity: "error", code: "chart-data-length", message: `Chart series ${seriesIndex + 1} values must match the categories length.`, slideId: slide.id });
    if (chart.type !== "line" && rawSeries.marker !== undefined) issues.push({ severity: "error", code: "invalid-chart-field", message: "Chart markers are only supported for line charts.", slideId: slide.id });
    if (chart.type === "pie" && series.values.some((value) => value < 0)) issues.push({ severity: "error", code: "pie-negative-value", message: "Pie chart values must be non-negative.", slideId: slide.id });
  });
  if (chart.type === "pie" && chart.series.length !== 1) issues.push({ severity: "error", code: "pie-single-series", message: "Pie charts require exactly one series.", slideId: slide.id });
  const scale = chart.type === "pie" ? undefined : chart.axes?.value?.scale;
  if (scale && scale !== "auto" && (!Number.isFinite(scale.min) || !Number.isFinite(scale.max) || scale.min >= scale.max || !Number.isFinite(scale.majorUnit) || scale.majorUnit <= 0)) {
    issues.push({ severity: "error", code: "invalid-chart-scale", message: "A fixed chart scale requires finite min < max and majorUnit > 0.", slideId: slide.id });
  }
}
