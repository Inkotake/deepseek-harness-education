...
export function detectCliAuth(options = {}) {
  ...
  for (const [providerId, probe] of Object.entries(AUTH_PROBES)) {
    ...
    try {
      ...
      out[providerId] = { authenticated: false, detail: cause.message };
      continue;
    }
    const verdict = probe.parse(result);
    out[providerId] = {
      authenticated: verdict === 'authenticated',
      detail: verdict ? 'cli reports an authenticated session' : 'cli has no authenticated session'
    };
  }
  out['github-pages'] = { authenticated: false, detail: 'github-pages deploys through the git origin remote' };
  cachedProbes = out;
  return out;
}
...
export function persistentEvidence(provider, context) {
  ...
  if (auth && auth.authenticated) return { evidence: true, reason: `bundled ${provider.cli} CLI is authenticated`, weight: 20 };
  return { evidence: false, reason: `no ${provider.label} configuration found and the bundled ${provider.cli || 'provider'} CLI is not authenticated`, weight: 0 };
}
...
export function planProviders(options) {
  ...
  for (const provider of registry.providers) {
    ...
    const htmlPolicy = provider.verification ? provider.verification.htmlPolicy : 'strict';
    ...
    eligible.push({
      ...
      ttlSecondsDefault: provider.capabilities.ttlSecondsDefault ?? null,
      reason,
  ...
  // Deterministic order: score, then cnPriority (a teacher in mainland China gets a better host
  // first among equals), then registry priority, then id.
  eligible.sort((a, b) => (b.score - a.score) || (b.cnPriority - a.cnPriority) || (b.priority - a.priority) || (a.id < b.id ? -1 : 1));

  return { mode, eligible, ineligible, disabled, generatedAt: new Date(now).toISOString() };
}
