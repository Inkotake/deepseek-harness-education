/**
 * Shared helpers for teacher-publish.
 *
 * House style follows `teacher/packages/artifact-cli/src/common.mjs`: `log` writes to stdout,
 * `warn`/`error` go to stderr so that `--json` mode can reserve stdout for exactly one JSON
 * document. Only `node:` builtins are used; the bundled runtime has no install step.
 */
...
export const LOG_PREFIX = '[teacher-publish] ';
...
/**
 * Teacher DSH writes state here and never into the installation directory.
 * Falls back to the user home directory when the variable is unset.
 */
export function resolveStateHome(env = process.env) {
  const explicit = env.TEACHER_DSH_HOME || env.TEACHER_HOME;
  const base = explicit ? path.resolve(explicit) : path.join(os.homedir(), '.teacher-dsh');
  try {
...
/**
 * Bundled deploy CLIs. In a Teacher DSH installation this is
 * `<resources>/teacher-runtime/deploy/node_modules/.bin`, exported as TEACHER_DEPLOY_HOME.
 * The relative fallbacks cover both the package layout inside the repository
 * (`<repo>/teacher/packages/publish-cli`) and the installed runtime layout
 * (`<runtime>/cli/publish-cli`).
 */
export function resolveDeployBinRoot(env = process.env) {
  return firstDirectory([
    env.TEACHER_DEPLOY_BIN,
    env.TEACHER_DEPLOY_HOME && path.join(env.TEACHER_DEPLOY_HOME, 'node_modules', '.bin'),
    env.TEACHER_DEPLOY_HOME && path.join(env.TEACHER_DEPLOY_HOME, '.bin'),
    // <repo>/teacher/packages/publish-cli -> <repo>/resources/teacher-runtime/deploy
    path.join(CLI_ROOT, '..', '..', '..', 'resources', 'teacher-runtime', 'deploy', 'node_modules', '.bin'),
    // <runtime>/cli/publish-cli -> <runtime>/deploy
    path.join(CLI_ROOT, '..', '..', 'deploy', 'node_modules', '.bin'),
    path.join(CLI_ROOT, '..', '..', '..', 'deploy', 'node_modules', '.bin')
  ]);
}
...
export const USER_AGENT = 'teacher-publish/0.1 (+https://github.com/anywhere-labs/dsh-desktop)';
      ...
      (response) => {
        ...
        response.on('end', () => {
          ...
          if (location && status >= 300 && status < 400 && redirects > 0) {
            ...
            httpRequest({ ...options, url: next, method: nextMethod, body: nextBody, redirects: redirects - 1 })
...
export function randomBoundary() {
  return '----teacherpublish' + crypto.randomBytes(16).toString('hex');
}
