...
const CHART_GROUPINGS = new Set(["clustered", "stacked", "percentStacked"]);
const CHART_LEGEND_POSITIONS = new Set(["right", "bottom"]);
const CHART_MARKERS = new Set(["auto", "diamond", "square", "circle", "triangle", "star", "x", "plus", "dash"]);
...
function validateChartPlan(chart: unknown, path: string) {
  if (!isRecord(chart) || !isStringArray(chart.categories) || chart.categories.length === 0 || !Array.isArray(chart.series) || chart.series.length === 0) {
    throw new Error(`${path} requires non-empty categories and series arrays.`);
  }
  if (typeof chart.type !== "string" || !CHART_TYPES.has(chart.type)) throw new Error(`${path}.type is unsupported: ${String(chart.type)}.`);
  if (chart.orientation !== undefined && chart.type !== "bar") throw new Error(`${path}.orientation is only supported for bar charts.`);
  if (chart.grouping !== undefined && chart.type !== "bar") throw new Error(`${path}.grouping is only supported for bar charts.`);
  if (chart.type === "bar" && chart.orientation !== undefined && (typeof chart.orientation !== "string" || !CHART_ORIENTATIONS.has(chart.orientation))) throw new Error(`${path}.orientation is unsupported: ${String(chart.orientation)}.`);
  if (chart.type === "bar" && chart.grouping !== undefined && (typeof chart.grouping !== "string" || !CHART_GROUPINGS.has(chart.grouping))) throw new Error(`${path}.grouping is unsupported: ${String(chart.grouping)}.`);
  if (chart.type === "pie" && chart.axes !== undefined) throw new Error(`${path}.axes is not supported for pie charts.`);
  if (chart.type === "pie" && chart.series.length !== 1) throw new Error(`${path}.series must contain exactly one series for pie charts.`);

  if (chart.legend !== undefined) {
    if (!isRecord(chart.legend)) throw new Error(`${path}.legend must be an object.`);
    if (chart.legend.visible !== undefined && typeof chart.legend.visible !== "boolean") throw new Error(`${path}.legend.visible must be a boolean.`);
    if (chart.legend.position !== undefined && (typeof chart.legend.position !== "string" || !CHART_LEGEND_POSITIONS.has(chart.legend.position))) throw new Error(`${path}.legend.position is unsupported: ${String(chart.legend.position)}.`);
  }

  if (chart.axes !== undefined) {
    if (!isRecord(chart.axes)) throw new Error(`${path}.axes must be an object.`);
    for (const axisName of ["category", "value"] as const) {
      const axis = chart.axes[axisName];
      if (axis === undefined) continue;
      if (!isRecord(axis)) throw new Error(`${path}.axes.${axisName} must be an object.`);
      if (axis.visible !== undefined && typeof axis.visible !== "boolean") throw new Error(`${path}.axes.${axisName}.visible must be a boolean.`);
      if (axis.labels !== undefined && typeof axis.labels !== "boolean") throw new Error(`${path}.axes.${axisName}.labels must be a boolean.`);
    }
    const valueAxis = chart.axes.value;
    if (isRecord(valueAxis) && valueAxis.scale !== undefined && valueAxis.scale !== "auto") {
      const scale = valueAxis.scale;
      if (!isRecord(scale) || typeof scale.min !== "number" || !Number.isFinite(scale.min) || typeof scale.max !== "number" || !Number.isFinite(scale.max) || scale.min >= scale.max || typeof scale.majorUnit !== "number" || !Number.isFinite(scale.majorUnit) || scale.majorUnit <= 0) {
        throw new Error(`${path}.axes.value.scale requires finite min < max and majorUnit > 0.`);
      }
    }
  }
  ...
  for (let seriesIndex = 0; seriesIndex < chart.series.length; seriesIndex += 1) {
    ...
    if (!isRecord(series)) throw new Error(`${path}.series[${seriesIndex}] must be an object.`);
    requireString(series.name, `${path}.series[${seriesIndex}].name`);
    if (!Array.isArray(series.values) || series.values.some((value) => typeof value !== "number" || !Number.isFinite(value))) throw new Error(`${path}.series[${seriesIndex}].values must be an array of finite numbers.`);
    if (series.values.length !== chart.categories.length) throw new Error(`${path}.series[${seriesIndex}].values length must match categories length.`);
    if (series.marker !== undefined && chart.type !== "line") throw new Error(`${path}.series[${seriesIndex}].marker is only supported for line charts.`);
    if (chart.type === "line" && series.marker !== undefined && series.marker !== false && (typeof series.marker !== "string" || !CHART_MARKERS.has(series.marker))) throw new Error(`${path}.series[${seriesIndex}].marker is unsupported: ${String(series.marker)}.`);
    if (chart.type === "pie" && series.values.some((value) => value < 0)) throw new Error(`${path}.series[${seriesIndex}].values must be non-negative for pie charts.`);
  }
}
...
function validateSlidePlan(slide: unknown, index: number, assetIds: ReadonlySet<string>, sourceIds: ReadonlySet<string>) {
  ...
  if (slide.chart !== undefined) {
    validateChartPlan(slide.chart, `${path}.chart`);
  }
